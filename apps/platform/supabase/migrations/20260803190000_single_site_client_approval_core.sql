-- GNR8 Single-Site Client Approval Core
-- Additive canonical workflow storage for operator client approval of the
...
alter table public.gnr8_single_site_migrations
  ...
  add constraint gnr8_single_site_migrations_state_stage_ck
    check (
      ...
      or (current_stage = 'improvement_content' and current_state in ('improvement_implementation_started', 'improvement_implementation_completed', 'improved_version_review_required', 'improved_preview_ready', 'content_review_required', 'content_approved', 'client_approval_required'))
  ...
  add constraint gnr8_single_site_migration_state_events_state_ck
    check (
      (from_state is null or from_state in (
        ...
        'improved_version_review_required', 'improved_preview_ready', 'content_review_required', 'content_approved',
        'client_approval_required', 'domain_readiness_required', 'domain_readiness_ready', 'subscription_required',
        'subscription_created', 'hosting_entitlement_ready', 'launch_approval_required', 'publish_ready', 'published',
        'rollback_available', 'migration_closed_out', 'migration_failed', 'migration_cancelled'
      ))
      and to_state in (
        'site_candidate_created', 'source_capture_started', 'source_capture_completed', 'source_capture_failed',
        'source_evidence_review_required', 'clone_generation_started', 'clone_generation_completed', 'clone_review_required',
...
create table if not exists public.gnr8_single_site_client_approvals (
  ...
  decision text null,
  client_approval_ready boolean not null default false,
  approved_with_limitations boolean not null default false,
  launch_approval_ready boolean not null default false,
  client_revision_required boolean not null default false,
  ...
  supersedes_client_approval_id uuid null references public.gnr8_single_site_client_approvals(id) on delete restrict,
  superseded_by_client_approval_id uuid null references public.gnr8_single_site_client_approvals(id) on delete restrict,
  ...
  constraint gnr8_single_site_client_approvals_status_ck
    check (status in (
      'not_required_yet',
      'required',
      'draft',
  ...
  constraint gnr8_single_site_client_approvals_review_status_ck
    check (improved_version_review_status in ('accepted', 'accepted_with_limitations')),
  constraint gnr8_single_site_client_approvals_content_status_ck
    check (content_approval_status in ('approved', 'approved_with_limitations')),
  constraint gnr8_single_site_client_approvals_decision_ck
  ...
  constraint gnr8_single_site_client_approvals_decision_required_ck
    check (
      status not in ('changes_requested', 'approved', 'approved_with_limitations', 'rejected', 'superseded', 'cancelled')
      or decision is not null
    ),
  constraint gnr8_single_site_client_approvals_aaf_decision_required_ck
    check (status not in ('approved', 'approved_with_limitations') or aaf_client_approval_decision_id is not null),
  constraint gnr8_single_site_client_approvals_ready_ck
    check (
      client_approval_ready = false
      or status in ('approved', 'approved_with_limitations')
    ),
  constraint gnr8_single_site_client_approvals_limited_ck
    check (
      approved_with_limitations = false
      or (status = 'approved_with_limitations' and jsonb_array_length(limitations_json) > 0)
    ),
  constraint gnr8_single_site_client_approvals_aaf_scope_ck
    check (
      (aaf_client_approval_scope is null or aaf_client_approval_scope = 'single_site_client_approval')
      and (aaf_client_approval_action is null or aaf_client_approval_action = 'approve_single_site_client_acceptance')
      and (aaf_client_approval_subject_type is null or aaf_client_approval_subject_type = 'single_site_improved_candidate_client_acceptance')
    ),
  constraint gnr8_single_site_client_approvals_reviewer_ck
    check (
      status not in ('changes_requested', 'approved', 'approved_with_limitations', 'rejected', 'superseded', 'cancelled')
  ...
  constraint gnr8_single_site_client_approvals_actor_type_ck
    check (reviewer_actor_type is null or reviewer_actor_type in ('human', 'system', 'provider', 'external_reference', 'ai_advisory')),
  constraint gnr8_single_site_client_approvals_json_shape_ck
    check (
      ...
      and jsonb_typeof(client_facing_summary_refs_json) = 'array'
      ...
      and jsonb_typeof(deferred_or_not_applied_recommendation_refs_json) = 'array'
  ...
  constraint gnr8_single_site_client_approvals_privacy_ck
  ...
  constraint gnr8_single_site_client_approvals_retention_ck
    check (retention_class in ('short_operational', 'mvp_operational', 'security', 'compliance_long', 'legal_hold')),
  constraint gnr8_single_site_client_approvals_nonempty_text_ck
    check (
  ...
  constraint gnr8_single_site_client_approvals_non_runtime_ck
    check (
...
create index if not exists idx_gnr8_single_site_client_approvals_latest
  on public.gnr8_single_site_client_approvals (migration_id, updated_at desc, created_at desc);

create unique index if not exists idx_gnr8_single_site_client_approvals_semantic
  on public.gnr8_single_site_client_approvals (
    migration_id,
    content_approval_id,
    improved_version_review_id,
    improved_candidate_site_version_ref,
    improved_runtime_artifact_ref,
    coalesce(supersedes_client_approval_id, '00000000-0000-0000-0000-000000000000'::uuid)
  );

create table if not exists public.gnr8_single_site_client_approval_refs (
  id uuid primary key default gen_random_uuid(),
  client_approval_id uuid not null references public.gnr8_single_site_client_approvals(id) on delete restrict,
  migration_id uuid not null references public.gnr8_single_site_migrations(id) on delete restrict,
  ...
  created_at timestamptz not null default now(),
  unique (idempotency_key),
  constraint gnr8_single_site_client_approval_refs_role_ck
    check (ref_role in (
      'migration',
      'client',
      'site',
  ...
  constraint gnr8_single_site_client_approval_refs_json_shape_ck
    check (jsonb_typeof(metadata_json) = 'object'),
  constraint gnr8_single_site_client_approval_refs_privacy_ck
    check (privacy_label in (
      'public_operational',
      'internal_operational',
      'client_confidential',
  ...
  constraint gnr8_single_site_client_approval_refs_retention_ck
    check (retention_class in ('short_operational', 'mvp_operational', 'security', 'compliance_long', 'legal_hold')),
  constraint gnr8_single_site_client_approval_refs_nonempty_text_ck
    check (
...
create unique index if not exists idx_gnr8_single_site_client_approval_refs_semantic
  on public.gnr8_single_site_client_approval_refs (
    client_approval_id,
    ref_role,
    source_system,
...
create table if not exists public.gnr8_single_site_client_approval_items (
  id uuid primary key default gen_random_uuid(),
  client_approval_id uuid not null references public.gnr8_single_site_client_approvals(id) on delete restrict,
  migration_id uuid not null references public.gnr8_single_site_migrations(id) on delete restrict,
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (client_approval_id, item_key),
  unique (idempotency_key),
  constraint gnr8_single_site_client_approval_items_category_ck
    check (category in (
      'business_acceptance',
      'content_acceptance',
      'brand_acceptance',
  ...
  constraint gnr8_single_site_client_approval_items_severity_ck
    check (severity in ('p0_blocker', 'p1_major', 'p2_minor', 'p3_note')),
  constraint gnr8_single_site_client_approval_items_status_ck
    check (status in ('open', 'resolved', 'accepted_limitation', 'superseded')),
  constraint gnr8_single_site_client_approval_items_blocking_ck
    check (
      blocks_approval = false
      or (status = 'open' and severity = 'p0_blocker' and accepted_limitation = false)
    ),
  constraint gnr8_single_site_client_approval_items_actor_type_ck
    check (reviewer_actor_type is null or reviewer_actor_type in ('human', 'system', 'provider', 'external_reference', 'ai_advisory')),
  constraint gnr8_single_site_client_approval_items_json_shape_ck
    check (
  ...
  constraint gnr8_single_site_client_approval_items_privacy_ck
    check (privacy_label in (
      'public_operational',
      'internal_operational',
      'client_confidential',
  ...
  constraint gnr8_single_site_client_approval_items_retention_ck
    check (retention_class in ('short_operational', 'mvp_operational', 'security', 'compliance_long', 'legal_hold')),
  constraint gnr8_single_site_client_approval_items_nonempty_text_ck
    check (
...
create table if not exists public.gnr8_single_site_client_approval_events (
  id uuid primary key default gen_random_uuid(),
  client_approval_id uuid not null references public.gnr8_single_site_client_approvals(id) on delete restrict,
  migration_id uuid not null references public.gnr8_single_site_migrations(id) on delete restrict,
  ...
  created_at timestamptz not null default now(),
  unique (client_approval_id, event_index),
  unique (idempotency_key),
  constraint gnr8_single_site_client_approval_events_action_ck
  ...
  constraint gnr8_single_site_client_approval_events_status_ck
    check (
  ...
  constraint gnr8_single_site_client_approval_events_actor_type_ck
    check (actor_type in ('human', 'system', 'provider', 'external_reference', 'ai_advisory')),
  constraint gnr8_single_site_client_approval_events_json_shape_ck
    check (
      jsonb_typeof(details_json) = 'object'
      and jsonb_typeof(limitations_json) = 'array'
      and jsonb_typeof(metadata_json) = 'object'
    ),
  constraint gnr8_single_site_client_approval_events_privacy_ck
    check (privacy_label in (
      'public_operational',
      'internal_operational',
      'client_confidential',
  ...
  constraint gnr8_single_site_client_approval_events_retention_ck
    check (retention_class in ('short_operational', 'mvp_operational', 'security', 'compliance_long', 'legal_hold')),
  constraint gnr8_single_site_client_approval_events_nonempty_text_ck
    check (
...
create table if not exists public.gnr8_single_site_client_approval_supersessions (
  id uuid primary key default gen_random_uuid(),
  migration_id uuid not null references public.gnr8_single_site_migrations(id) on delete restrict,
  superseded_client_approval_id uuid not null references public.gnr8_single_site_client_approvals(id) on delete restrict,
  replacement_client_approval_id uuid null references public.gnr8_single_site_client_approvals(id) on delete restrict,
  ...
  created_at timestamptz not null default now(),
  unique (idempotency_key),
  constraint gnr8_single_site_client_approval_supersessions_actor_type_ck
    check (actor_type in ('human', 'system', 'provider', 'external_reference', 'ai_advisory')),
  constraint gnr8_single_site_client_approval_supersessions_json_shape_ck
    check (jsonb_typeof(source_ref_json) = 'object' and jsonb_typeof(metadata_json) = 'object'),
  constraint gnr8_single_site_client_approval_supersessions_privacy_ck
    check (privacy_label in (
      'public_operational',
      'internal_operational',
      'client_confidential',
  ...
  constraint gnr8_single_site_client_approval_supersessions_retention_ck
    check (retention_class in ('short_operational', 'mvp_operational', 'security', 'compliance_long', 'legal_hold')),
  constraint gnr8_single_site_client_approval_supersessions_nonempty_text_ck
    check (
...
alter table public.gnr8_single_site_client_approvals enable row level security;
alter table public.gnr8_single_site_client_approval_refs enable row level security;
alter table public.gnr8_single_site_client_approval_items enable row level security;
alter table public.gnr8_single_site_client_approval_events enable row level security;
alter table public.gnr8_single_site_client_approval_supersessions enable row level security;

drop trigger if exists trg_gnr8_single_site_client_approval_refs_append_only
  on public.gnr8_single_site_client_approval_refs;
create trigger trg_gnr8_single_site_client_approval_refs_append_only
  before update or delete on public.gnr8_single_site_client_approval_refs
  for each row execute function public.gnr8_single_site_prevent_update_delete();

drop trigger if exists trg_gnr8_single_site_client_approval_events_append_only
  on public.gnr8_single_site_client_approval_events;
create trigger trg_gnr8_single_site_client_approval_events_append_only
  before update or delete on public.gnr8_single_site_client_approval_events
  for each row execute function public.gnr8_single_site_prevent_update_delete();

drop trigger if exists trg_gnr8_single_site_client_approval_supersessions_append_only
  on public.gnr8_single_site_client_approval_supersessions;
create trigger trg_gnr8_single_site_client_approval_supersessions_append_only
  before update or delete on public.gnr8_single_site_client_approval_supersessions
  for each row execute function public.gnr8_single_site_prevent_update_delete();
