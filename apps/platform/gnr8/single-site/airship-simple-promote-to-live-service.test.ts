...
test("simple promote rejects mismatched candidate and artifact refs", async () => {
  await assert.rejects(
    ...
    /airship_simple_promote_readiness_candidate_mismatch/,
  );
  ...
  await assert.rejects(
    () => promoteAirshipApprovedCandidateToLive(input(), deps({ reviewRecord: review({ candidateRuntimeArtifactId: "44444444-4444-4444-8444-444444444444" }) })),
    /airship_simple_promote_review_artifact_mismatch/,
  );
  ...
  await assert.rejects(
    ...
    /airship_simple_promote_candidate_version_artifact_mismatch/,
  );
  ...
  await assert.rejects(
    ...
    /airship_simple_promote_artifact_candidate_mismatch/,
  );
});

test("simple promote accepts matching runtime site id when ownership site label differs", async () => {
  const fakeDeps = deps();
  ...
  assert.equal(fakeDeps.events.includes("switch_active_pointer"), true);
});

test("simple promote rejects genuine runtime site mismatch before pointer switch", async () => {
  const fakeDeps = deps({
    versionSequence: [
      candidateVersion(),
      { ...candidateVersion(), siteId: OTHER_RUNTIME_SITE_ID },
    ],
  });

  await assert.rejects(
    () => promoteAirshipApprovedCandidateToLive(input(), fakeDeps),
    /airship_simple_promote_candidate_site_mismatch/,
  );
