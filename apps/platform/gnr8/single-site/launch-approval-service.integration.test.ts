...
import { LaunchApprovalService, type LaunchApprovalAafValidationResult } from "./launch-approval-service";
...
async function startDisposablePostgres() {
  ...
  const containerName = `gnr8-launch-approval-${process.pid}-${suffix}`;
  const database = `gnr8_launch_approval_${suffix.replace(/-/g, "")}`;
  const user = `gnr8_launch_approval_${suffix.replace(/-/g, "")}`;
...
function actor() {
  return { actorType: "human" as const, actorId: "launch-approval-integration-operator", actorRole: "migration_operator" };
}
...
function launchValidation(decisionId: string, launchApprovalId: string, status: "granted" | "granted_with_limitations" = "granted"): LaunchApprovalAafValidationResult {
  return {
    valid: true,
    status,
    scope: "single_site_launch_approval",
    subjectType: "single_site_launch_readiness_review",
    subjectId: launchApprovalId,
    approvalRequestId: `launch-request-for-${decisionId}`,
    approvalDecisionId: decisionId,
    evidencePackageId: `launch-evidence-for-${decisionId}`,
    limitations: status === "granted_with_limitations" ? [{ summary: "Accepted launch limitation." }] : [],
    blockerCodes: [],
...
async function seedCompletedExecution(writer: SingleSiteStateWriterRepository, suffix: string) {
  return writer.withTransaction(async (tx) => {
    const created = await writer.createMigration(tx, {
      ...
      clientId: randomUUID(),
      siteId: randomUUID(),
      sourceUrl: `https://launch-approval-${suffix}.example.test`,
      currentState: "improvement_implementation_completed",
      actor: { actorType: "human", actorId: "launch-approval-integration-operator" },
...
test("launch approval persists, projects, and preserves domain/billing/publish/runtime boundaries in disposable PostgreSQL", async () => {
  ...
  try {
    ...
    const launchDecisionId = randomUUID();
    ...
    const rls = await pool.query(
      ...
      order by relname
      `,
      [LAUNCH_APPROVAL_TABLES],
    );
    assert.equal(rls.rows.length, LAUNCH_APPROVAL_TABLES.length);
    ...
    const launchCreated = await launchService.createOrReuseLaunchApproval({
      ...
      evidencePackageRefsJson: [{ sourceRecordId: `launch-evidence-${suffix}`, refType: "single_site_launch_approval_evidence" }],
    ...
    await launchService.attachAafRequestRef({
      ...
      refType: "aaf_approval_request",
      sourceRecordId: `aaf-launch-request-${suffix}`,
      actor: actor(),
      correlationId: `corr-launch-aaf-request-${suffix}`,
      idempotencyKey: `idem-launch-aaf-request-${suffix}`,
    });
    await launchService.attachAafDecisionRef({
      ...
      refType: "aaf_approval_decision",
      sourceRecordId: launchDecisionId,
      launchApprovalValidation: launchValidation(launchDecisionId, launchCreated.launchApproval.id),
      actor: actor(),
      correlationId: `corr-launch-aaf-decision-${suffix}`,
      idempotencyKey: `idem-launch-aaf-decision-${suffix}`,
    });
    await launchService.addFinding({
      launchApprovalId: launchCreated.launchApproval.id,
      migrationId: seeded.migration.id,
      itemKey: "launch-ok",
      category: "qa_ready",
      severity: "p2_minor",
      status: "resolved",
      findingSummary: "Launch readiness checklist is ready for next-phase readiness work.",
      actor: actor(),
      correlationId: `corr-launch-item-${suffix}`,
      idempotencyKey: `idem-launch-item-${suffix}`,
    });
    await launchService.markReadyForReview({ launchApprovalId: launchCreated.launchApproval.id, actor: actor(), correlationId: `corr-launch-ready-${suffix}`, idempotencyKey: `idem-launch-ready-${suffix}` });
    await launchService.startReview({ launchApprovalId: launchCreated.launchApproval.id, actor: actor(), correlationId: `corr-launch-start-${suffix}`, idempotencyKey: `idem-launch-start-${suffix}` });
    const launchApproved = await launchService.approve({
      launchApprovalId: launchCreated.launchApproval.id,
      aafLaunchApprovalDecisionId: launchDecisionId,
      launchApprovalValidation: launchValidation(launchDecisionId, launchCreated.launchApproval.id),
      decisionSummaryJson: { decision: "launch approved" },
      actor: actor(),
      correlationId: `corr-launch-approve-${suffix}`,
      idempotencyKey: `idem-launch-approve-${suffix}`,
    });
    assert.equal(launchApproved.launchApproval.status, "approved");
    assert.equal(launchApproved.launchApproval.readiness_work_ready, true);
    assert.equal(launchApproved.launchApproval.launch_approval_granted, true);
    assert.equal(launchApproved.launchApproval.publish_activation_approval_granted, false);
    assert.equal(launchApproved.launchApproval.publish_readiness_not_granted, true);
    assert.equal(launchApproved.launchApproval.active_pointer_changed, false);
    assert.equal(launchApproved.launchApproval.runtime_artifacts_mutated, false);
    assert.equal(launchApproved.launchApproval.site_versions_mutated, false);
    ...
    assert.equal(model.launchApproval.latestLaunchApprovalId, launchCreated.launchApproval.id);
    assert.equal(model.launchApproval.status, "approved");
    assert.equal(model.launchApproval.contentApprovalRefs.contentApprovalId, created.contentApproval.id);
    assert.equal(model.launchApproval.clientApprovalRefs.clientApprovalId, clientCreated.clientApproval.id);
    assert.equal(model.launchApproval.aafRefs.decisionId, launchDecisionId);
    assert.equal(model.launchApproval.improvedCandidateRefs.siteVersionRef, "improved-version-" + suffix);
    assert.equal(model.launchApproval.readinessPlaceholders.domainReadinessRef, `domain-readiness-placeholder-${suffix}`);
    assert.equal(model.launchApproval.findingCount, 1);
    assert.equal(model.launchApproval.countsBySeverity.p2_minor, 1);
    assert.equal(model.launchApproval.countsByCategory.qa_ready, 1);
    assert.equal(model.launchApproval.unresolvedBlockerCount, 0);
    assert.equal(model.launchApproval.readinessWorkReady, true);
    assert.equal(model.launchApproval.publishReadinessNotGranted, true);
    ...
    assert.equal(model.launchApproval.activePointerChanged, false);
    assert.equal(model.launchApproval.runtimeMutatedByLaunchApproval, false);
    assert.equal(model.launchApproval.nextAction, "prepare_domain_billing_publish_readiness");
    assert.equal(model.workflowReadiness.launchApprovalRequired, false);

    await assert.rejects(
      () => pool.query("update public.gnr8_single_site_launch_approval_refs set ref_type = ref_type where launch_approval_id = $1::uuid", [launchCreated.launchApproval.id]),
      /append-only|Cannot update or delete/i,
    );
    await assert.rejects(
      () => pool.query("delete from public.gnr8_single_site_launch_approval_events where launch_approval_id = $1::uuid", [launchCreated.launchApproval.id]),
      /append-only|Cannot update or delete/i,
    );
