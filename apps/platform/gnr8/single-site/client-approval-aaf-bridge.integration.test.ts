...
async function startDisposablePostgres() {
  ...
  const containerName = `gnr8-client-approval-aaf-${process.pid}-${suffix}`;
  const database = `gnr8_client_approval_aaf_${suffix.replace(/-/g, "")}`;
  const user = `gnr8_client_approval_aaf_${suffix.replace(/-/g, "")}`;
...
function actor() {
  return { actorType: "human" as const, actorId: "client-approval-aaf-operator", actorRole: "migration_operator" };
}
...
async function seedAcceptedImprovedReview(writer: SingleSiteStateWriterRepository, suffix: string) {
  const seeded = await writer.withTransaction(async (tx) => {
    const created = await writer.createMigration(tx, {
      ...
      clientId: randomUUID(),
      siteId: randomUUID(),
      sourceUrl: `https://client-approval-aaf-${suffix}.example.test`,
      currentState: "improvement_implementation_completed",
      actor: { actorType: "human", actorId: "client-approval-aaf-operator" },
...
function bridgeInput(
  ...
  if (!contentApprovalDecisionId) throw new Error("content approval AAF decision id is required for client approval AAF bridge input");
  return {
    ...
    improvedCandidateRenderedSnapshotRef: ref("gnr8_single_site_client_approval_refs", `rendered-snapshot-${suffix}`, suffix),
    clientFacingSummaryRef: ref("gnr8_single_site_client_approval_refs", `client-facing-summary-${suffix}`, suffix),
    limitationsSummaryRef: ref("gnr8_single_site_client_approval_refs", `limitations-summary-${suffix}`, suffix),
    deferredOrNotAppliedRecommendationSummaryRef: ref("gnr8_single_site_client_approval_refs", `deferred-summary-${suffix}`, suffix),
    operatorAccountNotesRef: ref("gnr8_single_site_client_approval_refs", `operator-notes-${suffix}`, suffix),
    reviewerIdentityRef: ref("gnr8_identity_subjects", `client-reviewer-${suffix}`, suffix),
    reviewerRepresentativeRoleRef: ref("gnr8_account_roles", `client-representative-role-${suffix}`, suffix),
    limitations: [{ client: "Accepted client-visible caveat." }],
    deferredOrNotAppliedRecommendationRefs: [ref("gnr8_single_site_improvement_proposal_recommendations", `deferred-rec-${suffix}`, suffix)],
    auditTimelineRefs: [ref("gnr8_aaf_audit_events", `audit-timeline-${suffix}`, suffix)],
    operatorAccountNotes: [{ note: "Disposable MVP-33 bridge preparation only." }],
    actor: actor(),
    correlationId: `corr-client-aaf-bridge-${suffix}`,
    idempotencyKey: `idem-client-aaf-bridge-${suffix}`,
    policyVersion: "MVP-33",
  };
}

test("single-site client approval AAF bridge persists evidence/request and validates decisions in disposable PostgreSQL", async () => {
  ...
  try {
    ...
    const bridge = new SingleSiteClientApprovalAafBridge(aafWriter);
    const suffix = randomUUID().slice(0, 8);
    ...
    const input = bridgeInput(seeded, approvedContent.contentApproval, clientCreated.clientApproval.id, suffix);

    const prepared = await bridge.prepareClientApprovalRequest(input);
    assert.equal(prepared.evidencePackage.package_type, "single_site_client_approval_evidence");
    assert.equal(prepared.approvalRequest.scope, "single_site_client_approval");
    ...
    const replay = await bridge.prepareClientApprovalRequest(input);
    ...
    await assert.rejects(
      () =>
        bridge.prepareClientApprovalRequest({
          ...input,
          clientFacingSummaryRef: { ...input.clientFacingSummaryRef, sourceWatermark: "drifted-client-summary" },
        }),
    ...
    const wrongScopeRequest = await aafWriter.withTransaction((tx) =>
      aafWriter.createApprovalRequest(tx, {
        ...
        subjectId: `launch-readiness-${suffix}`,
    ...
    const wrongScope = await bridge.validateClientApprovalDecisionRef({ ...input, clientApprovalDecisionId: wrongScopeDecision.id });
    ...
    const grantedDecision = await aafWriter.withTransaction((tx) =>
      aafWriter.createApprovalDecision(tx, {
        ...
        decisionActorType: "human",
        decisionActorId: "client-approval-approver",
        decisionActorRole: "client_approver",
        policyVersion: "MVP-33",
        evidencePackageId: prepared.evidencePackage.id,
    ...
    const validation = await bridge.validateClientApprovalDecisionRef({
      ...input,
      clientApprovalDecisionId: grantedDecision.id,
      approvalRequestId: prepared.approvalRequest.id,
      evidencePackageId: prepared.evidencePackage.id,
    });
    ...
    const rejectedDecision = await aafWriter.withTransaction((tx) =>
      aafWriter.createApprovalDecision(tx, {
        ...
        status: "rejected",
        decisionActorType: "human",
        decisionActorId: "client-approval-rejecter",
        decisionActorRole: "client_approver",
        policyVersion: "MVP-33",
        evidencePackageId: prepared.evidencePackage.id,
    ...
    const rejected = await bridge.validateClientApprovalDecisionRef({ ...input, clientApprovalDecisionId: rejectedDecision.id });
    assert.equal(rejected.valid, false);
    assert.equal(rejected.status, "rejected");

    await assert.rejects(
      () =>
        clientService.attachAafDecisionRef({
          clientApprovalId: clientCreated.clientApproval.id,
          migrationId: seeded.migration.id,
    ...
    await clientService.attachAafDecisionRef({
      ...
      sourceRecordId: grantedDecision.id,
      sourceWatermark: prepared.semanticWatermark,
      clientApprovalValidation: validation,
      actor: actor(),
      correlationId: `corr-attach-validated-${suffix}`,
      idempotencyKey: `idem-attach-validated-${suffix}`,
    });
    const approved = await clientService.approveWithLimitations({
      clientApprovalId: clientCreated.clientApproval.id,
      aafClientApprovalDecisionId: grantedDecision.id,
      clientApprovalValidation: validation,
      actor: actor(),
      correlationId: `corr-service-approve-${suffix}`,
      idempotencyKey: `idem-service-approve-${suffix}`,
    });
    assert.equal(approved.clientApproval.status, "approved_with_limitations");
    ...
    assert.deepEqual(approved.clientApproval.launch_approval_granted, false);
    assert.deepEqual(approved.clientApproval.publish_activation_approval_granted, false);
    assert.deepEqual(approved.clientApproval.active_pointer_changed, false);
    assert.deepEqual(approved.clientApproval.runtime_artifacts_mutated, false);
    assert.deepEqual(approved.clientApproval.site_versions_mutated, false);
    assert.ok(Array.isArray(approved.clientApproval.limitations_json));
    assert.ok((approved.clientApproval.limitations_json as unknown[]).length > 0);

    const model = await reader.readByMigrationId(seeded.migration.id);
    assert.equal(model?.clientApproval.aafValidation.requestPrepared, true);
    assert.equal(model?.clientApproval.aafValidation.decisionState, "decision_validated");
    assert.equal(model?.clientApproval.status, "approved_with_limitations");
    assert.equal(model?.clientApproval.aafValidation.approvedWithLimitations, true);
    assert.equal(model?.clientApproval.aafValidation.limitationsCarriedForward, true);
    assert.equal(model?.clientApproval.launchApprovalReady, true);
    assert.equal(model?.clientApproval.launchApprovalGranted, false);
    assert.equal(model?.clientApproval.publishActivationApprovalGranted, false);

    const forbidden = await pool.query<{ client_forbidden: string; refs_forbidden: string; active_pointer_table: string | null }>(
      `
      select
        (select count(*)::text from public.gnr8_single_site_client_approvals where id = $1::uuid and (launch_approval_granted or publish_activation_approval_granted or active_pointer_changed or runtime_artifacts_mutated or site_versions_mutated)) as client_forbidden,
      ...
      `,
      [clientCreated.clientApproval.id, seeded.migration.id],
    );
    assert.equal(forbidden.rows[0]?.client_forbidden, "0");
