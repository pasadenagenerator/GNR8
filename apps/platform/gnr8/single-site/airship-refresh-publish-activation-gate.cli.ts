...
import { refreshAirshipPublishActivationGate } from "./airship-publish-activation-chain-service";
...
async function main(): Promise<void> {
  ...
  const output = await refreshAirshipPublishActivationGate({
    readinessPackageId,
    actorId,
    correlationId: readArg("correlation-id"),
    idempotencyKey: readArg("idempotency-key"),
  });
