...
test("operator panel renders dense read-only status without mutation buttons", () => {
  ...
  assert.equal(html.includes("collapsed debug detail"), true);
  assert.equal(html.includes("collapsed raw diagnostics"), true);
  assert.equal(html.includes("collapsed snapshot JSON/diff"), true);
  assert.equal(html.includes("collapsed evidence refs"), true);
  assert.equal(html.includes("collapsed audit timeline"), true);
...
test("operator panel renders accepted CHS internal MVP rehearsal summary read-only", () => {
  ...
  assert.equal(html.includes("Accepted MVP result"), true);
  assert.equal(html.includes("Live published candidate"), true);
  ...
  assert.equal(html.includes("Active pointer"), true);
  ...
  assert.equal(html.includes("Candidate preview is not the primary path."), true);
  assert.equal(html.includes("accepted candidate is now live through the active pointer"), true);
  assert.equal(html.includes("MVP Acceptance Evidence"), true);
