...
export async function buildWorkspaceOverviewModel(input?: {
  ...
  if (!selectedSnapshot) {
    ...
    return {
      ...
      executionIntentReadinessRecords: [],
      overview: {
  ...
  if (selectedSnapshot.source === "bundled_stable_import_snapshot") {
    ...
    const executionCandidateDiagnostics: string[] = [TWIN_EXECUTION_CANDIDATE_DIAGNOSTICS.STARTED];
    const executionCandidateRecords = buildExecutionCandidateRecords(executionPlanReadinessRecords);
    executionCandidateDiagnostics.push(TWIN_EXECUTION_CANDIDATE_DIAGNOSTICS.COMPLETED);
    ...
    const executionCandidatePackageDiagnostics: string[] = [
      TWIN_EXECUTION_CANDIDATE_PACKAGE_DIAGNOSTICS.STARTED,
    ];
    const executionCandidatePackageRecords = buildExecutionCandidatePackageRecords(
      executionCandidateRecords,
      executionCandidateReadinessRecords,
    );
    executionCandidatePackageDiagnostics.push(TWIN_EXECUTION_CANDIDATE_PACKAGE_DIAGNOSTICS.COMPLETED);
    if (hasExecutionCandidatePackageFallbackApplied(executionCandidatePackageRecords)) {
      executionCandidatePackageDiagnostics.push(
        TWIN_EXECUTION_CANDIDATE_PACKAGE_DIAGNOSTICS.FALLBACK_APPLIED,
      );
    }
    const executionCandidateAuthorizationRecords = buildExecutionCandidateAuthorizationRecords(
      executionCandidateRecords,
      executionCandidateReadinessRecords,
      executionCandidatePackageRecords,
    );
    const executionCandidateAuthorizationDiagnostics =
      getExecutionCandidateAuthorizationDiagnostics(executionCandidateAuthorizationRecords);
    const executionCandidateAuthorizationReadinessRecords =
      buildExecutionCandidateAuthorizationReadinessRecords(
        executionCandidateAuthorizationRecords,
        executionCandidateRecords,
        executionCandidateReadinessRecords,
    ...
    const executionCandidateAuthorizationReadinessDiagnostics =
      getExecutionCandidateAuthorizationReadinessDiagnostics(
        executionCandidateAuthorizationReadinessRecords,
      );
    ...
    const executionIntentReadinessDiagnostics: string[] = [
      TWIN_EXECUTION_INTENT_READINESS_DIAGNOSTICS.STARTED,
    ];
    const executionIntentReadinessRecords =
      generateTwinExecutionIntentReadinessRecords(executionIntentRecords);
    executionIntentReadinessDiagnostics.push(TWIN_EXECUTION_INTENT_READINESS_DIAGNOSTICS.COMPLETED);
    const diagnostics = [
