...
test("workspace overview model: twin overview and diagnostics render data from imported runtime evidence", async () => {
  ...
  assert.equal(Array.isArray(model.executionCandidateRecords), true);
  assert.equal(Array.isArray(model.executionCandidateReadinessRecords), true);
  assert.equal(Array.isArray(model.executionCandidatePackageRecords), true);
  assert.equal(Array.isArray(model.executionCandidateAuthorizationRecords), true);
  assert.equal(Array.isArray(model.executionCandidateAuthorizationReadinessRecords), true);
  assert.equal(Array.isArray(model.executionCandidateAuthorizationPackageRecords), true);
  ...
  assert.equal(Array.isArray(model.executionIntentReadinessRecords), true);
  ...
  assert.equal(model.diagnostics.includes("TWIN_EXECUTION_CANDIDATE_STARTED"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("TWIN_EXECUTION_CANDIDATE_COMPLETED"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("TWIN_EXECUTION_CANDIDATE_READINESS_STARTED"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("TWIN_EXECUTION_CANDIDATE_READINESS_COMPLETED"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("TWIN_EXECUTION_CANDIDATE_PACKAGE_STARTED"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("TWIN_EXECUTION_CANDIDATE_PACKAGE_COMPLETED"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("EXECUTION_CANDIDATE_AUTHORIZATION_CREATED"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("EXECUTION_CANDIDATE_AUTHORIZATION_BLOCKED"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("EXECUTION_CANDIDATE_AUTHORIZATION_READY"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("EXECUTION_CANDIDATE_AUTHORIZATION_READINESS_CREATED"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("EXECUTION_CANDIDATE_AUTHORIZATION_PACKAGE_CREATED"), model.sourceId !== null);
  assert.equal(
    model.diagnostics.includes("EXECUTION_CANDIDATE_AUTHORIZATION_PACKAGE_INCOMPLETE"),
    model.executionCandidateAuthorizationPackageRecords.some((entry) => entry.packageState === "package_incomplete"),
  );
  assert.equal(
    model.diagnostics.includes("EXECUTION_CANDIDATE_AUTHORIZATION_PACKAGE_READY"),
    model.executionCandidateAuthorizationPackageRecords.some((entry) => entry.packageState === "package_ready"),
  );
  assert.equal(
    model.diagnostics.includes("EXECUTION_CANDIDATE_AUTHORIZATION_READINESS_INCOMPLETE"),
    model.executionCandidateAuthorizationReadinessRecords.some((entry) => entry.readinessState === "incomplete"),
  );
  assert.equal(
    model.diagnostics.includes("EXECUTION_CANDIDATE_AUTHORIZATION_READINESS_NEARLY_READY"),
    model.executionCandidateAuthorizationReadinessRecords.some((entry) => entry.readinessState === "nearly_ready"),
  );
  assert.equal(
    model.diagnostics.includes("EXECUTION_CANDIDATE_AUTHORIZATION_READINESS_READY"),
    model.executionCandidateAuthorizationReadinessRecords.some((entry) => entry.readinessState === "ready"),
  );
  ...
  assert.equal(model.diagnostics.includes("TWIN_EXECUTION_INTENT_READINESS_STARTED"), model.sourceId !== null);
  assert.equal(model.diagnostics.includes("TWIN_EXECUTION_INTENT_READINESS_COMPLETED"), model.sourceId !== null);
  assert.equal(
  ...
  assert.equal(
    model.diagnostics.indexOf("TWIN_EXECUTION_INTENT_READINESS_STARTED") >
  ...
  assert.equal(
    model.diagnostics.indexOf("TWIN_EXECUTION_CANDIDATE_PACKAGE_STARTED") >
      model.diagnostics.indexOf("TWIN_EXECUTION_CANDIDATE_READINESS_COMPLETED"),
    model.sourceId !== null,
  );
  assert.equal(
    model.diagnostics.indexOf("EXECUTION_CANDIDATE_AUTHORIZATION_CREATED") >
      model.diagnostics.indexOf("TWIN_EXECUTION_CANDIDATE_PACKAGE_COMPLETED"),
    model.sourceId !== null,
  );
  assert.equal(
    model.diagnostics.indexOf("TWIN_EXECUTION_ARTIFACT_PREVIEW_STARTED") >
      model.diagnostics.indexOf("EXECUTION_CANDIDATE_AUTHORIZATION_PACKAGE_CREATED"),
    model.sourceId !== null,
  );
  assert.equal(
    model.diagnostics.indexOf("EXECUTION_CANDIDATE_AUTHORIZATION_PACKAGE_CREATED") >
      model.diagnostics.indexOf("EXECUTION_CANDIDATE_AUTHORIZATION_READINESS_CREATED"),
    model.sourceId !== null,
  );
  assert.equal(
    model.diagnostics.indexOf("EXECUTION_CANDIDATE_AUTHORIZATION_READINESS_CREATED") >
  ...
  assert.equal(
    model.diagnostics.indexOf("TWIN_EXECUTION_CANDIDATE_READINESS_STARTED") >
      model.diagnostics.indexOf("TWIN_EXECUTION_CANDIDATE_COMPLETED"),
    model.sourceId !== null,
  );
  assert.equal(
    model.diagnostics.indexOf("TWIN_EXECUTION_CANDIDATE_STARTED") >
      model.diagnostics.indexOf("TWIN_EXECUTION_PLAN_READINESS_CREATED"),
    model.sourceId !== null,
  );
...
test("workspace overview model: proposal candidates are present and remain read-only/non-executable", async () => {
  ...
  assert.equal(model.executionCandidateRecords.length, model.executionPlanReadinessRecords.length);
  assert.equal(model.executionCandidateRecords.every((entry) => entry.executionAllowed === false), true);
  assert.equal(model.executionCandidateRecords.every((entry) => entry.mutationAllowed === false), true);
  assert.equal(model.executionCandidateRecords.every((entry) => entry.publishingAllowed === false), true);
  assert.equal(model.executionCandidateRecords.every((entry) => entry.providerExecutionAllowed === false), true);
  assert.equal(
    model.executionCandidateRecords.every(
      (entry) => entry.governanceState === "execution_candidate_preview_only",
    ),
    true,
  );
  assert.equal(
    model.executionCandidateRecords.every((entry) => entry.blockedReasons.includes("governance_execution_blocked")),
    true,
  );
  assert.equal(model.executionCandidateReadinessRecords.length, model.executionCandidateRecords.length);
  assert.equal(model.executionCandidateReadinessRecords.every((entry) => entry.executionAllowed === false), true);
  assert.equal(model.executionCandidateReadinessRecords.every((entry) => entry.mutationAllowed === false), true);
  assert.equal(model.executionCandidateReadinessRecords.every((entry) => entry.publishingAllowed === false), true);
  assert.equal(
    model.executionCandidateReadinessRecords.every((entry) => entry.providerExecutionAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidateReadinessRecords.every(
      (entry) => entry.governanceState === "execution_candidate_readiness_preview_only",
    ),
    true,
  );
  assert.equal(
    model.executionCandidateReadinessRecords.every((entry) =>
      entry.blockedReasons.includes("governance_execution_blocked"),
    ),
    true,
  );
  assert.equal(model.executionCandidatePackageRecords.length, model.executionCandidateReadinessRecords.length);
  assert.equal(model.executionCandidatePackageRecords.every((entry) => entry.executionAllowed === false), true);
  assert.equal(model.executionCandidatePackageRecords.every((entry) => entry.mutationAllowed === false), true);
  assert.equal(model.executionCandidatePackageRecords.every((entry) => entry.publishingAllowed === false), true);
  assert.equal(
    model.executionCandidatePackageRecords.every((entry) => entry.providerExecutionAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidatePackageRecords.every(
      (entry) => entry.governanceState === "execution_candidate_package_preview_only",
    ),
    true,
  );
  assert.equal(
    model.executionCandidatePackageRecords.every((entry) =>
      entry.blockedReasons.includes("governance_execution_blocked"),
    ),
    true,
  );
  assert.equal(model.executionCandidateAuthorizationRecords.length, model.executionCandidatePackageRecords.length);
  assert.equal(model.executionCandidateAuthorizationRecords.every((entry) => entry.executionAllowed === false), true);
  assert.equal(model.executionCandidateAuthorizationRecords.every((entry) => entry.mutationAllowed === false), true);
  assert.equal(model.executionCandidateAuthorizationRecords.every((entry) => entry.publishingAllowed === false), true);
  assert.equal(
    model.executionCandidateAuthorizationRecords.every((entry) => entry.providerExecutionAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationRecords.every(
      (entry) => entry.governanceState === "execution_candidate_authorization_preview_only",
    ),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationRecords.every((entry) =>
      entry.blockedReasons.includes("governance_execution_blocked"),
    ),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationReadinessRecords.length,
    model.executionCandidateAuthorizationRecords.length,
  );
  assert.equal(
    model.executionCandidateAuthorizationReadinessRecords.every((entry) => entry.executionAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationReadinessRecords.every((entry) => entry.mutationAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationReadinessRecords.every((entry) => entry.publishingAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationReadinessRecords.every((entry) => entry.providerExecutionAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationReadinessRecords.every(
      (entry) => entry.governanceState === "execution_candidate_authorization_readiness_preview_only",
    ),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationReadinessRecords.every((entry) =>
      entry.blockedReasons.includes("governance_execution_blocked"),
    ),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationPackageRecords.length,
    model.executionCandidateAuthorizationReadinessRecords.length,
  );
  assert.equal(
    model.executionCandidateAuthorizationPackageRecords.every((entry) => entry.executionAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationPackageRecords.every((entry) => entry.mutationAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationPackageRecords.every((entry) => entry.publishingAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationPackageRecords.every((entry) => entry.providerExecutionAllowed === false),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationPackageRecords.every(
      (entry) => entry.governanceState === "execution_candidate_authorization_package_preview_only",
    ),
    true,
  );
  assert.equal(
    model.executionCandidateAuthorizationPackageRecords.every((entry) =>
      entry.blockedReasons.includes("governance_execution_blocked"),
    ),
    true,
  );
  ...
  assert.equal(model.executionIntentReadinessRecords.length, model.executionIntentRecords.length);
  assert.equal(model.executionIntentReadinessRecords.every((entry) => entry.executionAllowed === false), true);
  assert.equal(model.executionIntentReadinessRecords.every((entry) => entry.mutationAllowed === false), true);
  assert.equal(model.executionIntentReadinessRecords.every((entry) => entry.publishingAllowed === false), true);
  assert.equal(
    model.executionIntentReadinessRecords.every((entry) => entry.providerExecutionAllowed === false),
    true,
  );
  assert.equal(
    model.executionIntentReadinessRecords.every(
      (entry) => entry.governanceState === "execution_intent_readiness_preview_only",
    ),
    true,
  );
  ...
  const executionCandidateByTitle = new Map(
    model.executionCandidateRecords.map((entry) => [entry.proposalTitle, entry]),
  );
  const executionCandidateReadinessByTitle = new Map(
    model.executionCandidateReadinessRecords.map((entry) => [entry.proposalTitle, entry]),
  );
  const executionCandidatePackageByTitle = new Map(
    model.executionCandidatePackageRecords.map((entry) => [entry.proposalTitle, entry]),
  );
  const executionCandidateAuthorizationByTitle = new Map(
    model.executionCandidateAuthorizationRecords.map((entry) => [entry.proposalTitle, entry]),
  );
  const executionCandidateAuthorizationReadinessByProposalId = new Map(
    model.executionCandidateAuthorizationReadinessRecords.map((entry) => [entry.proposalId, entry]),
  );
  const executionCandidateAuthorizationPackageByProposalId = new Map(
    model.executionCandidateAuthorizationPackageRecords.map((entry) => [entry.proposalId, entry]),
  );
  ...
  const executionIntentReadinessByTitle = new Map(
    model.executionIntentReadinessRecords.map((entry) => [entry.proposalTitle, entry]),
  );
  ...
  if (conversionReadiness) {
    ...
    const executionCandidate = executionCandidateByTitle.get("Improve Homepage Conversion Flow");
    assert.equal(executionCandidate?.candidateState, "blocked_candidate");
    assert.equal(executionCandidate?.readinessState, planReadiness?.readinessState);
    assert.equal(executionCandidate?.readinessScore, 80);
    assert.equal(executionCandidate?.candidateType, "conversion_flow_execution_candidate");
    assert.deepEqual(executionCandidate?.candidateScope, ["homepage", "primary_conversion_path"]);
    assert.deepEqual(executionCandidate?.candidateArtifacts, [
      "conversion_review_document",
      "conversion_improvement_plan",
    ]);
    assert.deepEqual(executionCandidate?.blockedReasons, [
      "missing_conversion_baseline",
      "missing_design_evidence",
      "governance_execution_blocked",
    ]);
    assert.equal(executionCandidate?.executionAllowed, false);
    assert.equal(executionCandidate?.mutationAllowed, false);
    assert.equal(executionCandidate?.publishingAllowed, false);
    assert.equal(executionCandidate?.providerExecutionAllowed, false);
    assert.equal(executionCandidate?.governanceState, "execution_candidate_preview_only");
    const candidateReadiness = executionCandidateReadinessByTitle.get("Improve Homepage Conversion Flow");
    assert.equal(candidateReadiness?.readinessState, "incomplete");
    assert.equal(candidateReadiness?.readinessScore, 85);
    assert.deepEqual(candidateReadiness?.requirementsMet, [
      "execution_plan_present",
      "planning_artifacts_present",
      "conversion_plan_defined",
    ]);
    assert.deepEqual(candidateReadiness?.requirementsMissing, ["conversion_baseline", "design_evidence"]);
    assert.equal(candidateReadiness?.candidatePresent, true);
    assert.equal(candidateReadiness?.candidateArtifactsPresent, true);
    assert.equal(candidateReadiness?.executionAllowed, false);
    assert.equal(candidateReadiness?.mutationAllowed, false);
    assert.equal(candidateReadiness?.publishingAllowed, false);
    assert.equal(candidateReadiness?.providerExecutionAllowed, false);
    assert.equal(candidateReadiness?.governanceState, "execution_candidate_readiness_preview_only");
    const candidatePackage = executionCandidatePackageByTitle.get("Improve Homepage Conversion Flow");
    assert.equal(candidatePackage?.packageState, "package_incomplete");
    assert.equal(candidatePackage?.readinessState, candidateReadiness?.readinessState);
    assert.equal(candidatePackage?.readinessScore, 85);
    assert.equal(candidatePackage?.candidateType, executionCandidate?.candidateType);
    assert.deepEqual(candidatePackage?.includedComponents, [
      "execution_candidate",
      "execution_candidate_readiness",
      "candidate_scope",
      "candidate_artifacts",
    ]);
    assert.deepEqual(candidatePackage?.missingComponents, ["conversion_baseline", "design_evidence"]);
    assert.deepEqual(candidatePackage?.blockedReasons, [
      "missing_conversion_baseline",
      "missing_design_evidence",
      "governance_execution_blocked",
    ]);
    assert.equal(candidatePackage?.executionAllowed, false);
    assert.equal(candidatePackage?.mutationAllowed, false);
    assert.equal(candidatePackage?.publishingAllowed, false);
    assert.equal(candidatePackage?.providerExecutionAllowed, false);
    assert.equal(candidatePackage?.governanceState, "execution_candidate_package_preview_only");
    const candidateAuthorization = executionCandidateAuthorizationByTitle.get("Improve Homepage Conversion Flow");
    assert.equal(candidateAuthorization?.authorizationState, "authorization_blocked");
    assert.equal(candidateAuthorization?.readinessState, candidateReadiness?.readinessState);
    assert.equal(candidateAuthorization?.readinessScore, 85);
    assert.equal(candidateAuthorization?.authorizationType, "conversion_candidate_authorization");
    assert.deepEqual(candidateAuthorization?.requiredAuthorizations, [
      "operator_review",
      "conversion_baseline_review",
      "design_review",
    ]);
    assert.deepEqual(candidateAuthorization?.blockedReasons, [
      "missing_conversion_baseline",
      "missing_design_evidence",
      "governance_execution_blocked",
    ]);
    assert.equal(candidateAuthorization?.executionAllowed, false);
    assert.equal(candidateAuthorization?.mutationAllowed, false);
    assert.equal(candidateAuthorization?.publishingAllowed, false);
    assert.equal(candidateAuthorization?.providerExecutionAllowed, false);
    assert.equal(candidateAuthorization?.governanceState, "execution_candidate_authorization_preview_only");
    const candidateAuthorizationReadiness = candidateAuthorization
      ? executionCandidateAuthorizationReadinessByProposalId.get(candidateAuthorization.proposalId)
      : undefined;
    assert.equal(candidateAuthorizationReadiness?.readinessState, "incomplete");
    assert.equal(candidateAuthorizationReadiness?.readinessScore, 85);
    assert.equal(candidateAuthorizationReadiness?.authorizationPresent, true);
    assert.equal(candidateAuthorizationReadiness?.authorizationRequirementsPresent, true);
    assert.deepEqual(candidateAuthorizationReadiness?.requirementsMet, [
      "candidate_authorization_present",
      "candidate_authorization_scope_defined",
    ]);
    assert.deepEqual(candidateAuthorizationReadiness?.requirementsMissing, [
      "conversion_baseline",
      "design_evidence",
    ]);
    assert.deepEqual(candidateAuthorizationReadiness?.blockedReasons, [
      "missing_conversion_baseline",
      "missing_design_evidence",
      "governance_execution_blocked",
    ]);
    assert.equal(candidateAuthorizationReadiness?.executionAllowed, false);
    assert.equal(candidateAuthorizationReadiness?.mutationAllowed, false);
    assert.equal(candidateAuthorizationReadiness?.publishingAllowed, false);
    assert.equal(candidateAuthorizationReadiness?.providerExecutionAllowed, false);
    assert.equal(
      candidateAuthorizationReadiness?.governanceState,
      "execution_candidate_authorization_readiness_preview_only",
    );
    const candidateAuthorizationPackage = candidateAuthorization
      ? executionCandidateAuthorizationPackageByProposalId.get(candidateAuthorization.proposalId)
      : undefined;
    assert.equal(candidateAuthorizationPackage?.packageState, "package_incomplete");
    assert.equal(candidateAuthorizationPackage?.readinessState, "incomplete");
    assert.equal(candidateAuthorizationPackage?.readinessScore, 85);
    assert.equal(candidateAuthorizationPackage?.authorizationType, "conversion_candidate_authorization");
    assert.deepEqual(candidateAuthorizationPackage?.includedComponents, [
      "candidate_authorization",
      "candidate_authorization_readiness",
      "authorization_scope",
      "authorization_requirements",
    ]);
    assert.deepEqual(candidateAuthorizationPackage?.missingComponents, [
      "conversion_baseline",
      "design_evidence",
    ]);
    assert.deepEqual(candidateAuthorizationPackage?.blockedReasons, [
      "missing_conversion_baseline",
      "missing_design_evidence",
      "governance_execution_blocked",
    ]);
    assert.equal(candidateAuthorizationPackage?.executionAllowed, false);
    assert.equal(candidateAuthorizationPackage?.mutationAllowed, false);
    assert.equal(candidateAuthorizationPackage?.publishingAllowed, false);
    assert.equal(candidateAuthorizationPackage?.providerExecutionAllowed, false);
    assert.equal(
      candidateAuthorizationPackage?.governanceState,
    ...
    const executionIntentReadiness = executionIntentReadinessByTitle.get("Improve Homepage Conversion Flow");
    assert.equal(executionIntentReadiness?.readinessState, "intent_not_ready");
    assert.equal(executionIntentReadiness?.readinessScore, 80);
    assert.deepEqual(executionIntentReadiness?.requirementsMet, [
      "execution_intent_present",
      "conversion_intent_defined",
    ]);
    assert.deepEqual(executionIntentReadiness?.requirementsMissing, [
      "conversion_baseline",
      "design_evidence",
    ]);
  }
  ...
  if (messagingReadiness) {
    ...
    const executionCandidate = executionCandidateByTitle.get("Improve Homepage Quality and Messaging");
    assert.equal(executionCandidate?.candidateState, "candidate_ready_preview");
    assert.equal(executionCandidate?.readinessState, planReadiness?.readinessState);
    assert.equal(executionCandidate?.readinessScore, 90);
    assert.equal(executionCandidate?.candidateType, "homepage_messaging_execution_candidate");
    assert.deepEqual(executionCandidate?.candidateScope, ["homepage_hero", "homepage_messaging"]);
    assert.deepEqual(executionCandidate?.candidateArtifacts, [
    ...
    const candidateReadiness = executionCandidateReadinessByTitle.get("Improve Homepage Quality and Messaging");
    assert.equal(candidateReadiness?.readinessState, "nearly_ready");
    assert.equal(candidateReadiness?.readinessScore, 95);
    assert.deepEqual(candidateReadiness?.requirementsMet, [
      "execution_plan_present",
      "planning_artifacts_present",
      "content_plan_defined",
    ...
    assert.deepEqual(candidateReadiness?.requirementsMissing, ["design_evidence"]);
    assert.equal(candidateReadiness?.candidatePresent, true);
    assert.equal(candidateReadiness?.candidateArtifactsPresent, true);
    assert.equal(candidateReadiness?.executionAllowed, false);
    assert.equal(candidateReadiness?.mutationAllowed, false);
    ...
    assert.equal(candidateReadiness?.providerExecutionAllowed, false);
    assert.equal(candidateReadiness?.governanceState, "execution_candidate_readiness_preview_only");
    const candidatePackage = executionCandidatePackageByTitle.get("Improve Homepage Quality and Messaging");
    assert.equal(candidatePackage?.packageState, "package_ready");
    assert.equal(candidatePackage?.readinessState, candidateReadiness?.readinessState);
    assert.equal(candidatePackage?.readinessScore, 95);
    assert.equal(candidatePackage?.candidateType, executionCandidate?.candidateType);
    assert.deepEqual(candidatePackage?.includedComponents, [
      "execution_candidate",
      "execution_candidate_readiness",
    ...
    assert.deepEqual(candidatePackage?.missingComponents, ["design_evidence"]);
    assert.deepEqual(candidatePackage?.blockedReasons, [
      "missing_design_evidence",
      "governance_execution_blocked",
    ]);
    ...
    assert.equal(candidatePackage?.providerExecutionAllowed, false);
    assert.equal(candidatePackage?.governanceState, "execution_candidate_package_preview_only");
    const candidateAuthorization = executionCandidateAuthorizationByTitle.get("Improve Homepage Quality and Messaging");
    assert.equal(candidateAuthorization?.authorizationState, "authorization_ready_preview");
    assert.equal(candidateAuthorization?.readinessState, candidateReadiness?.readinessState);
    assert.equal(candidateAuthorization?.readinessScore, 95);
    assert.equal(candidateAuthorization?.authorizationType, "content_candidate_authorization");
    assert.deepEqual(candidateAuthorization?.requiredAuthorizations, ["operator_review", "design_review"]);
    assert.deepEqual(candidateAuthorization?.blockedReasons, [
      "missing_design_evidence",
      "governance_execution_blocked",
    ]);
    ...
    const candidateAuthorizationReadiness = candidateAuthorization
      ? executionCandidateAuthorizationReadinessByProposalId.get(candidateAuthorization.proposalId)
      : undefined;
    assert.equal(candidateAuthorizationReadiness?.readinessState, "nearly_ready");
    assert.equal(candidateAuthorizationReadiness?.readinessScore, 95);
    assert.equal(candidateAuthorizationReadiness?.authorizationPresent, true);
    assert.equal(candidateAuthorizationReadiness?.authorizationRequirementsPresent, true);
    assert.deepEqual(candidateAuthorizationReadiness?.requirementsMet, [
      "candidate_authorization_present",
    ...
    assert.deepEqual(candidateAuthorizationReadiness?.requirementsMissing, ["design_evidence"]);
    assert.deepEqual(candidateAuthorizationReadiness?.blockedReasons, [
      "missing_design_evidence",
      "governance_execution_blocked",
    ]);
    ...
    const candidateAuthorizationPackage = candidateAuthorization
      ? executionCandidateAuthorizationPackageByProposalId.get(candidateAuthorization.proposalId)
      : undefined;
    assert.equal(candidateAuthorizationPackage?.packageState, "package_ready");
    assert.equal(candidateAuthorizationPackage?.readinessState, "nearly_ready");
    assert.equal(candidateAuthorizationPackage?.readinessScore, 95);
    assert.equal(candidateAuthorizationPackage?.authorizationType, "content_candidate_authorization");
    assert.deepEqual(candidateAuthorizationPackage?.includedComponents, [
      "candidate_authorization",
      "candidate_authorization_readiness",
      "authorization_scope",
    ...
    assert.deepEqual(candidateAuthorizationPackage?.missingComponents, ["design_evidence"]);
    assert.deepEqual(candidateAuthorizationPackage?.blockedReasons, [
      "missing_design_evidence",
      "governance_execution_blocked",
    ]);
    ...
    const executionIntentReadiness = executionIntentReadinessByTitle.get("Improve Homepage Quality and Messaging");
    assert.equal(executionIntentReadiness?.readinessState, "intent_nearly_ready");
    assert.equal(executionIntentReadiness?.readinessScore, 95);
    assert.deepEqual(executionIntentReadiness?.requirementsMet, [
      "execution_intent_present",
      "content_intent_defined",
      "homepage_messaging_scope_defined",
    ]);
    assert.deepEqual(executionIntentReadiness?.requirementsMissing, ["design_evidence"]);
  }
  ...
  if (validationReadiness) {
    ...
    const executionCandidate = executionCandidateByTitle.get("Maintain Read-Only Validation Mode");
    assert.equal(executionCandidate?.candidateState, "candidate_ready_preview");
    assert.equal(executionCandidate?.readinessState, planReadiness?.readinessState);
    assert.equal(executionCandidate?.readinessScore, 100);
    assert.equal(executionCandidate?.candidateType, "validation_runtime_execution_candidate");
    assert.deepEqual(executionCandidate?.candidateScope, ["runtime_governance"]);
    assert.deepEqual(executionCandidate?.candidateArtifacts, ["validation_status_report"]);
    assert.deepEqual(executionCandidate?.blockedReasons, ["governance_execution_blocked"]);
    const candidateReadiness = executionCandidateReadinessByTitle.get("Maintain Read-Only Validation Mode");
    assert.equal(candidateReadiness?.readinessState, "ready");
    assert.equal(candidateReadiness?.readinessScore, 100);
    assert.deepEqual(candidateReadiness?.requirementsMet, [
      "execution_plan_present",
      "planning_artifacts_present",
      "governance_boundary_present",
    ...
    assert.deepEqual(candidateReadiness?.requirementsMissing, []);
    assert.equal(candidateReadiness?.candidatePresent, true);
    assert.equal(candidateReadiness?.candidateArtifactsPresent, true);
    assert.equal(candidateReadiness?.executionAllowed, false);
    assert.equal(candidateReadiness?.mutationAllowed, false);
    ...
    assert.equal(candidateReadiness?.providerExecutionAllowed, false);
    assert.equal(candidateReadiness?.governanceState, "execution_candidate_readiness_preview_only");
    const candidatePackage = executionCandidatePackageByTitle.get("Maintain Read-Only Validation Mode");
    assert.equal(candidatePackage?.packageState, "package_ready");
    assert.equal(candidatePackage?.readinessState, candidateReadiness?.readinessState);
    assert.equal(candidatePackage?.readinessScore, 100);
    assert.equal(candidatePackage?.candidateType, executionCandidate?.candidateType);
    assert.deepEqual(candidatePackage?.includedComponents, [
      "execution_candidate",
      "execution_candidate_readiness",
    ...
    assert.deepEqual(candidatePackage?.missingComponents, []);
    assert.deepEqual(candidatePackage?.blockedReasons, ["governance_execution_blocked"]);
    assert.equal(candidatePackage?.executionAllowed, false);
    assert.equal(candidatePackage?.mutationAllowed, false);
    assert.equal(candidatePackage?.publishingAllowed, false);
    assert.equal(candidatePackage?.providerExecutionAllowed, false);
    assert.equal(candidatePackage?.governanceState, "execution_candidate_package_preview_only");
    const candidateAuthorization = executionCandidateAuthorizationByTitle.get("Maintain Read-Only Validation Mode");
    assert.equal(candidateAuthorization?.authorizationState, "authorization_ready_preview");
    assert.equal(candidateAuthorization?.readinessState, candidateReadiness?.readinessState);
    assert.equal(candidateAuthorization?.readinessScore, 100);
    assert.equal(candidateAuthorization?.authorizationType, "governance_candidate_authorization");
    assert.deepEqual(candidateAuthorization?.requiredAuthorizations, ["operator_review"]);
    assert.deepEqual(candidateAuthorization?.blockedReasons, ["governance_execution_blocked"]);
    assert.equal(candidateAuthorization?.executionAllowed, false);
    assert.equal(candidateAuthorization?.mutationAllowed, false);
    assert.equal(candidateAuthorization?.publishingAllowed, false);
    assert.equal(candidateAuthorization?.providerExecutionAllowed, false);
    ...
    const candidateAuthorizationReadiness = candidateAuthorization
      ? executionCandidateAuthorizationReadinessByProposalId.get(candidateAuthorization.proposalId)
      : undefined;
    assert.equal(candidateAuthorizationReadiness?.readinessState, "ready");
    assert.equal(candidateAuthorizationReadiness?.readinessScore, 100);
    assert.equal(candidateAuthorizationReadiness?.authorizationPresent, true);
    assert.equal(candidateAuthorizationReadiness?.authorizationRequirementsPresent, true);
    assert.deepEqual(candidateAuthorizationReadiness?.requirementsMet, [
      "candidate_authorization_present",
    ...
    assert.deepEqual(candidateAuthorizationReadiness?.requirementsMissing, []);
    assert.deepEqual(candidateAuthorizationReadiness?.blockedReasons, ["governance_execution_blocked"]);
    assert.equal(candidateAuthorizationReadiness?.executionAllowed, false);
    assert.equal(candidateAuthorizationReadiness?.mutationAllowed, false);
    assert.equal(candidateAuthorizationReadiness?.publishingAllowed, false);
    assert.equal(candidateAuthorizationReadiness?.providerExecutionAllowed, false);
    ...
    const candidateAuthorizationPackage = candidateAuthorization
      ...
      : undefined;
    assert.equal(candidateAuthorizationPackage?.packageState, "package_ready");
    assert.equal(candidateAuthorizationPackage?.readinessState, "ready");
    assert.equal(candidateAuthorizationPackage?.readinessScore, 100);
    assert.equal(candidateAuthorizationPackage?.authorizationType, "governance_candidate_authorization");
    assert.deepEqual(candidateAuthorizationPackage?.includedComponents, [
      "candidate_authorization",
      "candidate_authorization_readiness",
      "authorization_scope",
    ...
    assert.deepEqual(candidateAuthorizationPackage?.missingComponents, []);
    assert.deepEqual(candidateAuthorizationPackage?.blockedReasons, ["governance_execution_blocked"]);
    assert.equal(candidateAuthorizationPackage?.executionAllowed, false);
    assert.equal(candidateAuthorizationPackage?.mutationAllowed, false);
    assert.equal(candidateAuthorizationPackage?.publishingAllowed, false);
    assert.equal(candidateAuthorizationPackage?.providerExecutionAllowed, false);
    ...
    const executionIntentReadiness = executionIntentReadinessByTitle.get("Maintain Read-Only Validation Mode");
    assert.equal(executionIntentReadiness?.readinessState, "intent_ready_preview");
    assert.equal(executionIntentReadiness?.readinessScore, 100);
    assert.deepEqual(executionIntentReadiness?.requirementsMet, [
      "execution_intent_present",
...
test("workspace overview model: observations include read-only runtime validation", async () => {
  ...
  if (model.sourceId === null) {
    ...
    assert.deepEqual(model.executionCandidateRecords, []);
    assert.deepEqual(model.executionCandidateReadinessRecords, []);
    assert.deepEqual(model.executionCandidatePackageRecords, []);
    assert.deepEqual(model.executionCandidateAuthorizationRecords, []);
    assert.deepEqual(model.executionCandidateAuthorizationReadinessRecords, []);
    assert.deepEqual(model.executionCandidateAuthorizationPackageRecords, []);
    ...
    assert.deepEqual(model.executionIntentReadinessRecords, []);
    return;
  }
...
test("workspace overview page source: renders required sections", async () => {
  ...
  assert.equal(source.includes("Execution Candidate"), true);
  assert.equal(source.includes("Execution Candidate Readiness"), true);
  assert.equal(source.includes("Execution Candidate Package"), true);
  assert.equal(source.includes("Execution Candidate Authorization"), true);
  assert.equal(source.includes("Execution Candidate Authorization Readiness"), true);
  assert.equal(source.includes("Execution Candidate Authorization Package"), true);
  ...
  assert.equal(source.includes("model.executionIntentReadinessRecords.map"), true);
  assert.equal(source.includes("intentReadinessRecord.proposalId"), true);
  assert.equal(source.includes("intentReadinessRecord.proposalTitle"), true);
  assert.equal(source.includes("intentReadinessRecord.readinessState"), true);
  assert.equal(source.includes("intentReadinessRecord.readinessScore"), true);
  assert.equal(source.includes("intentReadinessRecord.requirementsMet.map"), true);
  assert.equal(source.includes("intentReadinessRecord.requirementsMissing.map"), true);
  assert.equal(source.includes("intentReadinessRecord.executionAllowed"), true);
  assert.equal(source.includes("intentReadinessRecord.mutationAllowed"), true);
  assert.equal(source.includes("intentReadinessRecord.publishingAllowed"), true);
  assert.equal(source.includes("intentReadinessRecord.providerExecutionAllowed"), true);
  assert.equal(source.includes("intentReadinessRecord.governanceState"), true);
  assert.equal(source.includes("intentReadinessRecord.summary"), true);
  ...
  assert.equal(source.includes("model.executionCandidateRecords.map"), true);
  assert.equal(source.includes("candidateRecord.candidateId"), true);
  assert.equal(source.includes("candidateRecord.proposalTitle"), true);
  assert.equal(source.includes("candidateRecord.candidateState"), true);
  assert.equal(source.includes("candidateRecord.readinessState"), true);
  assert.equal(source.includes("candidateRecord.readinessScore"), true);
  assert.equal(source.includes("candidateRecord.candidateType"), true);
  assert.equal(source.includes("candidateRecord.candidateScope.map"), true);
  assert.equal(source.includes("candidateRecord.candidateArtifacts.map"), true);
  assert.equal(source.includes("candidateRecord.candidateRequirements.map"), true);
  assert.equal(source.includes("candidateRecord.blockedReasons.map"), true);
  assert.equal(source.includes("candidateRecord.executionAllowed"), true);
  assert.equal(source.includes("candidateRecord.mutationAllowed"), true);
  assert.equal(source.includes("candidateRecord.publishingAllowed"), true);
  assert.equal(source.includes("candidateRecord.providerExecutionAllowed"), true);
  assert.equal(source.includes("candidateRecord.governanceState"), true);
  assert.equal(source.includes("candidateRecord.summary"), true);
  assert.equal(source.includes("Candidate Scope"), true);
  assert.equal(source.includes("Candidate Artifacts"), true);
  assert.equal(source.includes("Candidate Requirements"), true);
  assert.equal(source.includes("model.executionCandidateReadinessRecords.map"), true);
  assert.equal(source.includes("readinessRecord.candidateId"), true);
  assert.equal(source.includes("readinessRecord.candidatePresent"), true);
  assert.equal(source.includes("readinessRecord.candidateArtifactsPresent"), true);
  assert.equal(source.includes("readinessRecord.blockedReasons.map"), true);
  assert.equal(source.includes("model.executionCandidatePackageRecords.map"), true);
  assert.equal(source.includes("packageRecord.candidateId"), true);
  assert.equal(source.includes("packageRecord.proposalTitle"), true);
  assert.equal(source.includes("packageRecord.packageState"), true);
  assert.equal(source.includes("packageRecord.readinessState"), true);
  assert.equal(source.includes("packageRecord.readinessScore"), true);
  assert.equal(source.includes("packageRecord.candidateType"), true);
  assert.equal(source.includes("packageRecord.includedComponents.map"), true);
  assert.equal(source.includes("packageRecord.missingComponents.map"), true);
  assert.equal(source.includes("packageRecord.blockedReasons.map"), true);
  assert.equal(source.includes("packageRecord.executionAllowed"), true);
  assert.equal(source.includes("packageRecord.mutationAllowed"), true);
  assert.equal(source.includes("packageRecord.publishingAllowed"), true);
  assert.equal(source.includes("packageRecord.providerExecutionAllowed"), true);
  assert.equal(source.includes("packageRecord.governanceState"), true);
  assert.equal(source.includes("packageRecord.summary"), true);
  assert.equal(source.includes("model.executionCandidateAuthorizationRecords.map"), true);
  assert.equal(source.includes("authorizationRecord.proposalId"), true);
  assert.equal(source.includes("authorizationRecord.proposalTitle"), true);
  assert.equal(source.includes("authorizationRecord.authorizationState"), true);
  assert.equal(source.includes("authorizationRecord.readinessState"), true);
  assert.equal(source.includes("authorizationRecord.readinessScore"), true);
  assert.equal(source.includes("authorizationRecord.authorizationType"), true);
  assert.equal(source.includes("authorizationRecord.requiredAuthorizations.map"), true);
  assert.equal(source.includes("authorizationRecord.blockedReasons.map"), true);
  assert.equal(source.includes("authorizationRecord.governanceState"), true);
  assert.equal(source.includes("authorizationRecord.executionAllowed"), true);
  assert.equal(source.includes("authorizationRecord.mutationAllowed"), true);
  assert.equal(source.includes("authorizationRecord.publishingAllowed"), true);
  assert.equal(source.includes("authorizationRecord.providerExecutionAllowed"), true);
  assert.equal(source.includes("authorizationRecord.summary"), true);
  assert.equal(source.includes("Required Authorizations"), true);
  assert.equal(source.includes("model.executionCandidateAuthorizationReadinessRecords.map"), true);
  assert.equal(source.includes("authorizationReadinessRecord.proposalId"), true);
  assert.equal(source.includes("authorizationReadinessRecord.readinessState"), true);
  assert.equal(source.includes("authorizationReadinessRecord.readinessScore"), true);
  assert.equal(source.includes("authorizationReadinessRecord.authorizationPresent"), true);
  assert.equal(source.includes("authorizationReadinessRecord.authorizationRequirementsPresent"), true);
  assert.equal(source.includes("authorizationReadinessRecord.requirementsMet.map"), true);
  assert.equal(source.includes("authorizationReadinessRecord.requirementsMissing.map"), true);
  assert.equal(source.includes("authorizationReadinessRecord.blockedReasons.map"), true);
  assert.equal(source.includes("authorizationReadinessRecord.governanceState"), true);
  assert.equal(source.includes("authorizationReadinessRecord.executionAllowed"), true);
  assert.equal(source.includes("authorizationReadinessRecord.mutationAllowed"), true);
  assert.equal(source.includes("authorizationReadinessRecord.publishingAllowed"), true);
  assert.equal(source.includes("authorizationReadinessRecord.providerExecutionAllowed"), true);
  assert.equal(source.includes("authorizationReadinessRecord.summary"), true);
  assert.equal(source.includes("model.executionCandidateAuthorizationPackageRecords.map"), true);
  assert.equal(source.includes("authorizationPackageRecord.proposalId"), true);
  assert.equal(source.includes("authorizationPackageRecord.packageState"), true);
  assert.equal(source.includes("authorizationPackageRecord.readinessState"), true);
  assert.equal(source.includes("authorizationPackageRecord.readinessScore"), true);
  assert.equal(source.includes("authorizationPackageRecord.authorizationType"), true);
  assert.equal(source.includes("authorizationPackageRecord.includedComponents.map"), true);
  assert.equal(source.includes("authorizationPackageRecord.missingComponents.map"), true);
  assert.equal(source.includes("authorizationPackageRecord.blockedReasons.map"), true);
  assert.equal(source.includes("authorizationPackageRecord.governanceState"), true);
  assert.equal(source.includes("authorizationPackageRecord.executionAllowed"), true);
  assert.equal(source.includes("authorizationPackageRecord.mutationAllowed"), true);
  assert.equal(source.includes("authorizationPackageRecord.publishingAllowed"), true);
  assert.equal(source.includes("authorizationPackageRecord.providerExecutionAllowed"), true);
  assert.equal(source.includes("authorizationPackageRecord.summary"), true);
...
test("workspace overview page source: planning sections render in runtime order", async () => {
  ...
  const executionCandidateIndex = source.indexOf("Execution Candidate");
  const executionCandidateReadinessIndex = source.indexOf("Execution Candidate Readiness");
  const executionCandidatePackageIndex = source.indexOf("Execution Candidate Package");
  const executionCandidateAuthorizationIndex = source.indexOf("Execution Candidate Authorization");
  const executionCandidateAuthorizationReadinessIndex = source.indexOf("Execution Candidate Authorization Readiness");
  const executionCandidateAuthorizationPackageIndex = source.indexOf("Execution Candidate Authorization Package");
  ...
  assert.equal(executionCandidateIndex > executionPlanReadinessIndex, true);
  assert.equal(executionCandidateReadinessIndex > executionCandidateIndex, true);
  assert.equal(executionCandidatePackageIndex > executionCandidateReadinessIndex, true);
  assert.equal(executionCandidateAuthorizationIndex > executionCandidatePackageIndex, true);
  assert.equal(executionCandidateAuthorizationReadinessIndex > executionCandidateAuthorizationIndex, true);
  assert.equal(executionCandidateAuthorizationPackageIndex > executionCandidateAuthorizationReadinessIndex, true);
  assert.equal(executionArtifactIndex > executionCandidateAuthorizationPackageIndex, true);
  assert.equal(opportunityRankingIndex > executionArtifactIndex, true);
  assert.equal(opportunityRankingIndex > executionCandidateIndex, true);
});
