...
export const REAL_TWIN_PREVIEW_FIXTURE_ID = "real-site-01" as const;
...
function toTwinIdentityFromImport(input: {
  ...
  return {
    ...
    workspaceId: "workspace_fixture_validation_runtime",
    sourceImportId: `import_${input.fixtureId}_${shortSpecHash}`,
    requestId: input.requestId,
  };
}

export async function buildRealSiteTwinPreviewModel() {
  const fixture = readValidationFixtureSpec(REAL_TWIN_PREVIEW_FIXTURE_ID);
  const fixtureRootDirAbs = validationFixtureDirAbs(REAL_TWIN_PREVIEW_FIXTURE_ID);
  const requestId = `twin-preview-real-${REAL_TWIN_PREVIEW_FIXTURE_ID}`;
  ...
  const twin = buildWebsiteDigitalTwin({
    ...
    generatedBy: "twin_preview_real_page_v1",
  });
  ...
  if (!storedTwin) {
    throw new Error("TWIN_PREVIEW_REAL_RUNTIME_INVARIANT: stored twin missing for real fixture site version");
  }
  ...
  return {
    fixtureId: fixture.fixtureId,
    sourceImportId: storedTwin.metadata.sourceImportId,
    sourceSiteVersionId: storedTwin.metadata.sourceSiteVersionId,
    generatedAt: storedTwin.metadata.generatedAt,
    overview,
    diagnostics,
  };
}
