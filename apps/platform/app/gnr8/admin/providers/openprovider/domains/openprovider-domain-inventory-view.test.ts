...
const VIEW_FILE = new URL("./openprovider-domain-inventory-view.tsx", import.meta.url);

test("openprovider domain inventory view source: empty inventory render", async () => {
  const source = await readFile(VIEW_FILE, "utf8");
  assert.equal(source.includes("No domains found in current Openprovider sandbox account."), true);
});

test("openprovider domain inventory view source: loaded inventory table render", async () => {
  const source = await readFile(VIEW_FILE, "utf8");
  assert.equal(source.includes("domain"), true);
  assert.equal(source.includes("status"), true);
  assert.equal(source.includes("expiration"), true);
  assert.equal(source.includes("nameservers count"), true);
});

test("openprovider domain inventory view source: diagnostics collapse", async () => {
  const source = await readFile(VIEW_FILE, "utf8");
...
test("openprovider domain inventory view source: summary cards", async () => {
  const source = await readFile(VIEW_FILE, "utf8");
...
test("openprovider domain inventory view source: badge rendering states", async () => {
  const source = await readFile(VIEW_FILE, "utf8");
