  ...
  return (
    ...
    >
      <section style={{ border: '1px solid #dbe6f1', borderRadius: 12, background: '#fff', padding: 14 }}>
        <p style={{ marginTop: 0, marginBottom: 0, fontSize: 13, color: '#334155' }}>
          {canRunMigrations
      ...
      <section style={{ marginTop: 12, border: '1px solid #dbe6f1', borderRadius: 12, background: '#fff', padding: 14 }}>
        <form method='get' style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'end' }}>
          ...
          {isAdminView ? <input type='hidden' name='admin_view' value='1' /> : null}
          ...
          <Link
            href={
              requestedAgencyId
                ? `/gnr8/agency?agency=${encodeURIComponent(requestedAgencyId)}${isAdminView ? '&admin_view=1' : ''}`
                : `/gnr8/agency${isAdminView ? '?admin_view=1' : ''}`
            }
            style={{
              height: 38,
              padding: '8px 12px',
      ...
      <section style={{ marginTop: 16, border: '1px solid #dbe6f1', borderRadius: 12, background: '#fff', padding: 16 }}>
        ...
        >
          ...
          <MetricCard
            label='Total Margin'
            value={formatMoney(readModel.summary.total_margin)}
            tone={readModel.summary.total_margin >= 0 ? 'good' : 'warn'}
          />
        </div>
