...
test("airship single-site editor renders CHS summary, live link, and AI improvement status", () => {
  ...
  assert.equal(html.includes("Create online Airship session"), true);
  ...
  assert.equal(html.includes("id=\"airship-local-proof-flow\""), true);
  assert.equal((html.match(/<button[^>]*disabled=""/g) ?? []).length >= 4, true);
