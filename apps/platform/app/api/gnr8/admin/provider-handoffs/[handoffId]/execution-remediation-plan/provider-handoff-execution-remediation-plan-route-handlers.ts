...
type ProviderHandoffExecutionRemediationPlanRouteDependencies = {
  ...
  createRuntimeProviderExecutionRemediationPlan: typeof createRuntimeProviderExecutionRemediationPlan;
...
export function createProviderHandoffExecutionRemediationPlanRouteHandlers(
  deps: Partial<ProviderHandoffExecutionRemediationPlanRouteDependencies> = {},
) {
  const resolvedDeps: ProviderHandoffExecutionRemediationPlanRouteDependencies = {
    ...
    createRuntimeProviderExecutionRemediationPlan,
  ...
  return {
    async GET(_request: Request, context: { params: Promise<{ handoffId: string }> }): Promise<Response> {
      try {
        ...
        const executionRemediationPlan = resolvedDeps.createRuntimeProviderExecutionRemediationPlan({
          handoffId: normalizedHandoffId,
          executionPreconditionsLedger,
        });

        const status = !normalizedHandoffId ? 400 : !handoffArtifact ? 404 : 200;
        return Response.json({ executionRemediationPlan, executionAllowed: false, executionBlocked: true, intentOnly: true }, { status });
      } catch (error) {
        ...
        return Response.json({ executionRemediationPlan, executionAllowed: false, executionBlocked: true, intentOnly: true }, { status: 500 });
      }
    },
  };
}
