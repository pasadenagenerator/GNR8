...
type ProviderHandoffDryRunJobPlanRouteDeps = {
  ...
  createRuntimeProviderDryRunJobPlan: typeof createRuntimeProviderDryRunJobPlan;
...
export function createProviderHandoffDryRunJobPlanRouteHandlers(deps: Partial<ProviderHandoffDryRunJobPlanRouteDeps> = {}) {
  const resolvedDeps: ProviderHandoffDryRunJobPlanRouteDeps = {
    requireSuperadminUserId,
    getProviderExecutionHandoffByHandoffId,
    createRuntimeProviderDryRunJobPlan,
  ...
  return {
    async GET(_request: Request, context: { params: Promise<{ handoffId: string }> }): Promise<Response> {
      try {
        ...
        const dryRunJobPlan = resolvedDeps.createRuntimeProviderDryRunJobPlan({
          handoffId: normalizedHandoffId,
          handoffArtifact,
        });
        const status = !normalizedHandoffId ? 400 : !handoffArtifact ? 404 : 200;
        return Response.json({ dryRunJobPlan, executionAllowed: false, executionBlocked: true, intentOnly: true }, { status });
      } catch (error) {
        ...
        return Response.json({ dryRunJobPlan, executionAllowed: false, executionBlocked: true, intentOnly: true }, { status: 500 });
      }
    },
  };
}
