...
test("provider handoff execution readiness gate route: no execution controls", async () => {
  ...
  const response = await handlers.GET(new Request("http://localhost/api/gnr8/admin/provider-handoffs/handoff_1/execution-readiness-gate"), {
  ...
  assert.equal(body.executionReadinessGate.executionAllowed, false);
  assert.equal(body.executionReadinessGate.executionBlocked, true);
  assert.equal(body.executionReadinessGate.gateStatus, "theoretically_ready_but_disabled");
  assert.equal(body.executionReadinessGate.diagnostics.includes("EXECUTION_READINESS_GATE_THEORETICALLY_READY_DISABLED"), true);
});
