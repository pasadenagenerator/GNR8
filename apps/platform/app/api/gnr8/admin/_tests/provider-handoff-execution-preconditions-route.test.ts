...
test("provider handoff execution preconditions route: returns evidence-only ledger and executionAllowed false", async () => {
  ...
  const response = await handlers.GET(new Request("http://localhost/api/gnr8/admin/provider-handoffs/handoff_1/execution-preconditions"), {
  ...
  assert.equal(body.executionPreconditionsLedger.executionAllowed, false);
  assert.equal(body.executionPreconditionsLedger.executionBlocked, true);
  assert.equal(body.executionPreconditionsLedger.overallStatus, "satisfied_but_execution_disabled");
});
