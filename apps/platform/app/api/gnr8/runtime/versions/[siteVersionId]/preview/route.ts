...
function styleAnchorAndImage(anchor){if(!anchor||!anchor.style)return;anchor.style.display="block";anchor.style.width="100%";anchor.style.overflow="hidden";var img=anchor.querySelector("img");if(img&&img.style){img.style.display="block";img.style.width="100%";img.style.height="auto";img.style.maxWidth="100%";img.style.objectFit="contain";}}
...
function wireControls(moduleEl,host,pageCount){if(!moduleEl||!host||pageCount<2)return{wired:false,activePageIndex:0};var prev=moduleEl.querySelector(".prev,.arrow-prev,[data-dir='prev'],[aria-label*='prev' i]");var next=moduleEl.querySelector(".next,.arrow-next,[data-dir='next'],[aria-label*='next' i]");if(!prev||!next)return{wired:false,activePageIndex:0};var activeIndex=0;prev.addEventListener("click",function(ev){ev.preventDefault();activeIndex=Math.max(0,activeIndex-1);setActivePage(host,activeIndex);});next.addEventListener("click",function(ev){ev.preventDefault();activeIndex=Math.min(pageCount-1,activeIndex+1);setActivePage(host,activeIndex);});return{wired:true,activePageIndex:activeIndex};}
