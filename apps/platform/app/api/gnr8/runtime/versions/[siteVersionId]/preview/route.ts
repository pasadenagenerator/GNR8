...
var BUILDER_NATIVE_CIRCLE_MIN_SIZE=40;
var BUILDER_NATIVE_MIN_B=105;
var BUILDER_NATIVE_MIN_SATURATION=25;
...
var MAP_FALLBACK_EXCLUSION_TOKENS=["gnr8-preview-map-fallback","gnr8-map-fallback","osmap-fallback","map-fallback","map-shell","leaflet"];
var falsePositiveLogCount=0;
var suppressedFalsePositiveLogCount=0;
var MAX_FALSE_POSITIVE_LOGS=20;
var finalVisualPassTimer=null;
var nativeDiscoverySnapshotEmitted=false;
...
if(isFalsePositiveLoggableCandidate(el,aggregate)){
...
function isFalsePositiveLoggableCandidate(el,aggregateInput){
if(!el||!el.getAttribute)return false;
if(isFallbackElement(el))return false;
var tag=String(el.tagName||"").toLowerCase();
if(tag==="body"||tag==="html"||tag==="style")return false;
var aggregate=String(aggregateInput||"").toLowerCase();
if(aggregate.indexOf("gnr8-gallery-pages")>=0)return false;
if(hasAnyToken(aggregate,MAP_FALLBACK_EXCLUSION_TOKENS))return false;
if((tag==="div"||tag==="span"||tag==="p")&&!hasAnyToken(aggregate,["prev","next","slider","carousel","slick-prev","slick-next","swiper-button-prev","swiper-button-next","arrow-left","arrow-right","chevron-left","chevron-right"]))return false;
if(tag==="a"||tag==="button"||el.getAttribute("role")==="button")return true;
if(hasAnyToken(aggregate,["prev","next","slider","carousel","slick-prev","slick-next","swiper-button-prev","swiper-button-next","arrow-left","arrow-right","chevron-left","chevron-right"]))return true;
return false;
}
...
var nativeBuilderDetection=detectNativeBuilderBackToTopCandidate(el);
return !!nativeBuilderDetection.isNativeBuilder;
}
...
function detectNativeBuilderBackToTopCandidate(el){
if(!el||!el.getAttribute)return{isNativeBuilder:false,detectionReason:"none",iconEvidence:"none",runtimeBehaviorDetected:false,positionEvidence:"none",className:"",tag:"",id:"",matchedSignals:[],delayedRuntimeDetected:false};
var rect=getRectSafe(el);
var style=window.getComputedStyle?window.getComputedStyle(el):null;
var viewportW=Math.max(window.innerWidth||0,document.documentElement&&document.documentElement.clientWidth||0);
var viewportH=Math.max(window.innerHeight||0,document.documentElement&&document.documentElement.clientHeight||0);
...
var circularShape=rect.width>=BUILDER_NATIVE_CIRCLE_MIN_SIZE&&rect.height>=BUILDER_NATIVE_CIRCLE_MIN_SIZE&&Math.abs(rect.width-rect.height)<=20;
...
var isBlueLike=!!bg&&bg.b>=BUILDER_NATIVE_MIN_B&&bg.b>=bg.r+12&&bg.b>=bg.g+12&&(Math.max(bg.r,bg.g,bg.b)-Math.min(bg.r,bg.g,bg.b))>=BUILDER_NATIVE_MIN_SATURATION;
...
var className=String(el.className||"");
var tag=String(el.tagName||"").toLowerCase();
var idRaw=String(el.id||"");
var id=String(el.id||"").toLowerCase();
var tokenMatched=hasAnyToken(id+" "+String(className||"").toLowerCase(),BACK_TO_TOP_TOKENS);
var matchedSignals=[];
if(fixedLike)matchedSignals.push("fixed_like");
if(bottomRightish)matchedSignals.push("bottom_right");
if(circularShape||radiusCircular)matchedSignals.push("circular");
if(isBlueLike)matchedSignals.push("blue_like");
if(iconEvidence!=="none")matchedSignals.push("up_icon");
if(runtime.runtimeBehaviorDetected)matchedSignals.push(runtime.reason);
if(tokenMatched)matchedSignals.push("token");
var delayedRuntimeDetected=runtime.runtimeBehaviorDetected&&runtime.reason==="runtime_initialized_scroll_control";
...
return{
isNativeBuilder:isOriginal,
detectionReason:matchedSignals.join("|")||"none",
...
delayedRuntimeDetected:delayedRuntimeDetected
};
}
...
var val=parseFloat(String(input||""));
return Number.isFinite(val)?val:0;
}
function buildNativeDiscoveryCandidate(node){
if(!node||!node.getAttribute)return null;
var style=window.getComputedStyle?window.getComputedStyle(node):null;
var rect=getRectSafe(node);
var position=style?String(style.position||""):"";
var clickable=node.tagName==="A"||node.tagName==="BUTTON"||node.getAttribute("role")==="button"||typeof node.onclick==="function"||String(node.getAttribute("onclick")||"").trim().length>0||(style&&(style.cursor==="pointer"));
var visible=isVisibleElement(node);
var viewportW=Math.max(window.innerWidth||0,document.documentElement&&document.documentElement.clientWidth||0);
var viewportH=Math.max(window.innerHeight||0,document.documentElement&&document.documentElement.clientHeight||0);
var nearBottomRight=rect.width>0&&rect.height>0&&rect.right>=viewportW-260&&rect.bottom>=viewportH-260;
var radiusRaw=style?String(style.borderRadius||""):"";
var radiusNum=parseFloat(radiusRaw);
var looksCircular=rect.width>=28&&rect.height>=28&&Math.abs(rect.width-rect.height)<=16&&(!Number.isNaN(radiusNum)&&radiusNum>=14||/999|50%/.test(radiusRaw));
var txt=((node.textContent||"")+" "+(node.getAttribute("aria-label")||"")+" "+(node.getAttribute("title")||"")+" "+(node.className||"")+" "+(node.id||"")).toLowerCase();
var hasSvg=!!(node.querySelector&&node.querySelector("svg"));
var svgNode=hasSvg&&node.querySelector?node.querySelector("svg"):null;
var svgClass=svgNode?String(svgNode.getAttribute("class")||""):"";
var svgText=truncateText(svgNode&&svgNode.textContent?svgNode.textContent:"",120);
var svgPathCount=node.querySelectorAll?node.querySelectorAll("svg path").length:0;
var hasIconClass=/icon|fa-|chevron|arrow|angle-up|caret-up/.test(txt);
var hasClickHandlerSignal=typeof node.onclick==="function"||String(node.getAttribute("onclick")||"").trim().length>0;
var looksLikeUpControl=hasAnyPhrase(txt,["up","top","totop","scrolltop","back to top","back-to-top","onepage-up","scroll-up"])||UP_ARROW_CONTENT_REGEX.test(txt)||hasSvg||hasIconClass;
var topPx=toNumeric(style?style.top:"");
var rightPx=toNumeric(style?style.right:"");
var bottomPx=toNumeric(style?style.bottom:"");
var hasBottomRightPositioning=rightPx>0&&bottomPx>0;
var likelyFloating=position==="fixed"||position==="sticky"||hasBottomRightPositioning||toNumeric(style?style.zIndex:"")>0||topPx>0;
if(!clickable&&!likelyFloating)return null;
return{
element:node,
...
textContent:truncateText(node.textContent||"",160),
innerHTML:truncateText(node.innerHTML||"",180),
computedStyle:{
...
zIndex:style?String(style.zIndex||""):"",
bottom:style?String(style.bottom||""):"",
right:style?String(style.right||""):"",
top:style?String(style.top||""):"",
left:style?String(style.left||""):"",
...
color:style?String(style.color||""):"",
transform:style?String(style.transform||""):""
},
boundingClientRect:{x:rect.left,y:rect.top,width:rect.width,height:rect.height},
hasSvg:hasSvg,
svgClass:svgClass,
svgText:svgText,
svgPathCount:svgPathCount,
hasIconClass:hasIconClass,
hasClickHandlerSignal:hasClickHandlerSignal,
isVisible:visible,
isNearBottomRight:nearBottomRight,
looksCircular:looksCircular,
looksLikeUpControl:looksLikeUpControl,
isGnr8Fallback:isFallbackElement(node)
};
}
function scoreNativeDiscoveryCandidate(candidate){
if(!candidate)return-9999;
var style=candidate.computedStyle||{};
var score=0;
if(candidate.isVisible&&(style.position==="fixed"||style.position==="sticky")&&candidate.isNearBottomRight)score+=120;
if(candidate.looksCircular)score+=40;
if(candidate.hasSvg||candidate.hasIconClass||candidate.looksLikeUpControl)score+=30;
if(candidate.hasClickHandlerSignal||candidate.tag==="a"||candidate.tag==="button")score+=20;
if(candidate.isGnr8Fallback)score-=120;
return score;
}
function discoverLikelyNativeBackToTopCandidates(){
var nodes=Array.prototype.slice.call(document.querySelectorAll("a,button,[role='button'],[onclick],[style*='cursor:pointer'],[style*='cursor: pointer'],[style*='position:fixed'],[style*='position: fixed'],[style*='position:sticky'],[style*='position: sticky'],[style*='z-index'],[style*='bottom'],[style*='right']"));
var seen=[];
var rows=[];
for(var i=0;i<nodes.length;i+=1){
var node=nodes[i];
...
seen.push(node);
var candidate=buildNativeDiscoveryCandidate(node);
if(!candidate)continue;
rows.push(candidate);
}
rows.sort(function(a,b){return scoreNativeDiscoveryCandidate(b)-scoreNativeDiscoveryCandidate(a);});
if(!nativeDiscoverySnapshotEmitted){
nativeDiscoverySnapshotEmitted=true;
emit("PREVIEW_BACK_TO_TOP_NATIVE_DISCOVERY_SNAPSHOT",{
siteVersionId:payload.siteVersionId,
candidateCount:rows.length,
candidates:rows.slice(0,20).map(function(row){
return{
tag:row.tag,
id:row.id,
...
boundingClientRect:row.boundingClientRect,
hasSvg:row.hasSvg,
svgClass:row.svgClass,
svgText:row.svgText,
svgPathCount:row.svgPathCount,
hasIconClass:row.hasIconClass,
hasClickHandlerSignal:row.hasClickHandlerSignal,
isVisible:row.isVisible,
isNearBottomRight:row.isNearBottomRight,
looksCircular:row.looksCircular,
looksLikeUpControl:row.looksLikeUpControl,
isGnr8Fallback:row.isGnr8Fallback
};
}),
correlationKey:correlationKey("native_discovery_snapshot")
});
}
return rows;
}
...
if(!isFalsePositiveLoggableCandidate(nodeForExclude,excludeAggregate))continue;
...
var nativeDiscovery=discoverLikelyNativeBackToTopCandidates();
var discoveredBestNative=nativeDiscovery.length>0?nativeDiscovery[0]:null;
if(!existing&&discoveredBestNative&&!discoveredBestNative.isGnr8Fallback&&discoveredBestNative.isVisible&&discoveredBestNative.isNearBottomRight&&discoveredBestNative.looksCircular&&discoveredBestNative.looksLikeUpControl){
existing=discoveredBestNative.element;
}
var existingElementFound=!!existing;
var nativeBuilderDetection=existing?detectNativeBuilderBackToTopCandidate(existing):null;
...
var detectionReason=existingElementFound?(nativeBuilderDetection&&nativeBuilderDetection.isNativeBuilder?"NATIVE_BUILDER_RUNTIME_ORIGINAL":"EXISTING_MARKUP"):"NO_ELEMENT";
...
if(existing&&nativeBuilderDetection&&nativeBuilderDetection.isNativeBuilder){
emit("PREVIEW_BACK_TO_TOP_NATIVE_BUILDER_DETECTED",{
siteVersionId:payload.siteVersionId,
detectionReason:nativeBuilderDetection.detectionReason,
tag:nativeBuilderDetection.tag,
id:nativeBuilderDetection.id,
className:nativeBuilderDetection.className,
matchedSignals:nativeBuilderDetection.matchedSignals,
delayedRuntimeDetected:nativeBuilderDetection.delayedRuntimeDetected,
correlationKey:correlationKey("native_builder_detected_"+passName)
});
}
if(existing){
...
finalButtonSource:"native_builder",
...
emit("PREVIEW_BACK_TO_TOP_DEDUPED",{siteVersionId:payload.siteVersionId,passName:passName,candidateCount:dedupedOriginal.candidateCount,originalCandidateCount:dedupedOriginal.originalCandidateCount,fallbackCandidateCount:dedupedOriginal.fallbackCandidateCount,fallbackInjectionPrevented:fallbackCandidateCount>0||!!fallbackCandidate,finalButtonSource:"native_builder",hiddenDuplicateCount:dedupedOriginal.hiddenDuplicateCount,visibleCandidateCountAfter:dedupedOriginal.visibleCandidateCountAfter,excludedFalsePositiveCount:dedupedOriginal.excludedFalsePositiveCount,nativeBuilderCandidateCount:dedupedOriginal.originalCandidateCount,fallbackSuppressedCount:dedupedOriginal.fallbackCandidateCount,finalVisibleBackToTopCount:dedupedOriginal.visibleCandidateCountAfter,correlationKey:correlationKey("deduped_original_"+passName)});
...
var rescannedDetection=detectNativeBuilderBackToTopCandidate(rescannedOriginal);
emit("PREVIEW_BACK_TO_TOP_NATIVE_BUILDER_DETECTED",{
siteVersionId:payload.siteVersionId,
detectionReason:rescannedDetection.detectionReason,
tag:rescannedDetection.tag,
id:rescannedDetection.id,
className:rescannedDetection.className,
matchedSignals:rescannedDetection.matchedSignals,
delayedRuntimeDetected:rescannedDetection.delayedRuntimeDetected,
correlationKey:correlationKey("native_builder_rescan_"+passName)
});
var dedupedRescanned=dedupeCandidates(rescannedOriginal,passName);
emit("PREVIEW_BACK_TO_TOP_DEDUPED",{siteVersionId:payload.siteVersionId,passName:passName,candidateCount:dedupedRescanned.candidateCount,originalCandidateCount:dedupedRescanned.originalCandidateCount,fallbackCandidateCount:dedupedRescanned.fallbackCandidateCount,fallbackInjectionPrevented:true,finalButtonSource:"native_builder",hiddenDuplicateCount:dedupedRescanned.hiddenDuplicateCount,visibleCandidateCountAfter:dedupedRescanned.visibleCandidateCountAfter,excludedFalsePositiveCount:dedupedRescanned.excludedFalsePositiveCount,nativeBuilderCandidateCount:dedupedRescanned.originalCandidateCount,fallbackSuppressedCount:dedupedRescanned.fallbackCandidateCount,finalVisibleBackToTopCount:dedupedRescanned.visibleCandidateCountAfter,correlationKey:correlationKey("deduped_original_rescan_"+passName)});
var suppressedRescanFallbackCount=0;
for(var rs=0;rs<rescannedCandidates.length;rs+=1){
if(isFallbackElement(rescannedCandidates[rs])&&suppressFallbackCandidate(rescannedCandidates[rs]))suppressedRescanFallbackCount+=1;
}
if(suppressedRescanFallbackCount>0){
emit("PREVIEW_BACK_TO_TOP_NATIVE_SUPPRESSED_FALLBACK",{
siteVersionId:payload.siteVersionId,
nativeCandidateSummary:{tag:rescannedDetection.tag,id:rescannedDetection.id,className:rescannedDetection.className,detectionReason:rescannedDetection.detectionReason},
fallbackSuppressedCount:suppressedRescanFallbackCount,
correlationKey:correlationKey("native_suppressed_fallback_"+passName)
});
}
return;
}
...
emit("PREVIEW_BACK_TO_TOP_DEDUPED",{siteVersionId:payload.siteVersionId,passName:passName,candidateCount:dedupedFallback.candidateCount,originalCandidateCount:dedupedFallback.originalCandidateCount,fallbackCandidateCount:dedupedFallback.fallbackCandidateCount,fallbackInjectionPrevented:false,finalButtonSource:"fallback",hiddenDuplicateCount:dedupedFallback.hiddenDuplicateCount,visibleCandidateCountAfter:dedupedFallback.visibleCandidateCountAfter,excludedFalsePositiveCount:dedupedFallback.excludedFalsePositiveCount,nativeBuilderCandidateCount:dedupedFallback.originalCandidateCount,fallbackSuppressedCount:0,finalVisibleBackToTopCount:dedupedFallback.visibleCandidateCountAfter,correlationKey:correlationKey("deduped_fallback_"+passName)});
...
function hasClearUpEvidence(el){
if(!el||!el.getAttribute)return false;
var text=((el.textContent||"")+" "+(el.getAttribute("aria-label")||"")+" "+(el.getAttribute("title")||"")+" "+(el.className||"")+" "+(el.id||"")).toLowerCase();
if(hasAnyPhrase(text,["up","chevron-up","angle-up","arrow-up","top","totop","scrolltop","back to top","back-to-top","icon-up"]))return true;
if(el.querySelector&&el.querySelector("svg,path,polyline,polygon,[class*='chevron-up'],[class*='angle-up'],[class*='arrow-up'],i.fa-chevron-up,i.fa-angle-up,[aria-label*='up' i],[title*='up' i]"))return true;
return false;
}
function isBottomAreaOrTokenMatch(el){
if(!el)return false;
var rect=getRectSafe(el);
var viewportW=Math.max(window.innerWidth||0,document.documentElement&&document.documentElement.clientWidth||0);
var viewportH=Math.max(window.innerHeight||0,document.documentElement&&document.documentElement.clientHeight||0);
var inBottomRight=rect.width>0&&rect.height>0&&rect.right>=viewportW-240&&rect.bottom>=viewportH-260;
var inBottomBand=rect.width>0&&rect.height>0&&rect.bottom>=viewportH-180;
var text=((el.textContent||"")+" "+(el.getAttribute&&el.getAttribute("aria-label")||"")+" "+(el.getAttribute&&el.getAttribute("title")||"")+" "+(el.className||"")+" "+(el.id||"")).toLowerCase();
var tokenMatch=hasAnyPhrase(text,["scroll","top","up","totop","scrolltop","back-to-top","back to top"]);
return inBottomRight||inBottomBand||tokenMatch;
}
function isOriginalLikeVisibleControl(el){
if(!el||!el.getAttribute)return false;
if(isFallbackElement(el))return false;
if(!isVisibleElement(el))return false;
var tag=String(el.tagName||"").toLowerCase();
var style=window.getComputedStyle?window.getComputedStyle(el):null;
var clickable=tag==="a"||tag==="button"||el.getAttribute("role")==="button"||typeof el.onclick==="function"||String(el.getAttribute("onclick")||"").trim().length>0||(style&&style.cursor==="pointer");
if(!clickable)return false;
var floatingLike=style&&(style.position==="fixed"||style.position==="sticky"||style.position==="absolute");
if(!floatingLike)return false;
if(!isBottomAreaOrTokenMatch(el))return false;
if(!hasClearUpEvidence(el))return false;
return true;
}
function getVisibleFallbackElements(){
var seen=[];
var nodes=Array.prototype.slice.call(document.querySelectorAll("#"+BACK_TO_TOP_FALLBACK_ID+",[data-gnr8-backtotop-fallback='1']"));
var visible=[];
for(var i=0;i<nodes.length;i+=1){
var node=nodes[i];
if(!node||seen.indexOf(node)>=0)continue;
seen.push(node);
if(isVisibleElement(node))visible.push(node);
}
return visible;
}
function runFinalVisualPass(passName){
var fallbackNodesBefore=getVisibleFallbackElements();
var allControls=Array.prototype.slice.call(document.querySelectorAll("a,button,[role='button'],[onclick],[style*='cursor: pointer'],[style*='cursor:pointer']"));
var visibleOriginalLike=[];
for(var i=0;i<allControls.length;i+=1){
var el=allControls[i];
if(isOriginalLikeVisibleControl(el))visibleOriginalLike.push(el);
}
var fallbackRemovedCount=0;
var fallbackHiddenCount=0;
var finalButtonSource="fallback";
if(visibleOriginalLike.length>0){
for(var r=0;r<fallbackNodesBefore.length;r+=1){
var fallbackNode=fallbackNodesBefore[r];
if(fallbackNode&&fallbackNode.parentNode){
fallbackNode.parentNode.removeChild(fallbackNode);
fallbackRemovedCount+=1;
}else if(suppressFallbackCandidate(fallbackNode)){
fallbackHiddenCount+=1;
}
}
finalButtonSource="original";
if(fallbackNodesBefore.length>0){
var nativeEl=visibleOriginalLike[0];
emit("PREVIEW_BACK_TO_TOP_NATIVE_SUPPRESSED_FALLBACK",{
siteVersionId:payload.siteVersionId,
nativeCandidateSummary:{tag:String(nativeEl.tagName||"").toLowerCase(),id:String(nativeEl.id||""),className:String(nativeEl.className||"")},
fallbackSuppressedCount:fallbackRemovedCount+fallbackHiddenCount,
correlationKey:correlationKey("native_suppressed_fallback_final_"+passName)
});
}
}else{
var allFallbackNodes=Array.prototype.slice.call(document.querySelectorAll("#"+BACK_TO_TOP_FALLBACK_ID+",[data-gnr8-backtotop-fallback='1']"));
var keeper=allFallbackNodes.length>0?allFallbackNodes[0]:null;
for(var d=0;d<allFallbackNodes.length;d+=1){
var dupeFallback=allFallbackNodes[d];
if(!dupeFallback||dupeFallback===keeper)continue;
if(dupeFallback.parentNode){
dupeFallback.parentNode.removeChild(dupeFallback);
fallbackRemovedCount+=1;
}else if(suppressFallbackCandidate(dupeFallback)){
fallbackHiddenCount+=1;
}
}
if(keeper)ensureVisible(keeper);
finalButtonSource="fallback";
}
var finalVisibleFallbackCount=getVisibleFallbackElements().length;
var finalVisibleBackToTopCount=visibleOriginalLike.length>0?visibleOriginalLike.length:finalVisibleFallbackCount;
emit("PREVIEW_BACK_TO_TOP_FINAL_VISUAL_PASS",{
siteVersionId:payload.siteVersionId,
passName:passName,
visibleOriginalLikeCount:visibleOriginalLike.length,
visibleFallbackCountBefore:fallbackNodesBefore.length,
fallbackRemovedCount:fallbackRemovedCount,
fallbackHiddenCount:fallbackHiddenCount,
finalVisibleBackToTopCount:finalVisibleBackToTopCount,
finalButtonSource:finalButtonSource,
correlationKey:correlationKey("final_visual_pass_"+passName)
});
}
function scheduleFinalVisualPass(passName,delayMs){
if(finalVisualPassTimer)clearTimeout(finalVisualPassTimer);
finalVisualPassTimer=setTimeout(function(){runFinalVisualPass(passName);},delayMs);
}
function schedulePass(name,delayMs){
setTimeout(function(){runDedupePass(name);scheduleFinalVisualPass(name,80);},delayMs);
}
try{
runDedupePass("initial");
scheduleFinalVisualPass("initial",80);
...
scheduleFinalVisualPass("mutation_observer",120);
return;
}
}
}
...
scheduleFinalVisualPass("final_settle",2200);
}catch(_err){}
})();</script>`;
  if (input.html.includes("</body>")) {
    return input.html.replace("</body>", `${script}</body>`);
