...
function normalizeGalleryListMarkers(moduleEl,pagesHost){var suppressedCount=0;if(!moduleEl||!moduleEl.querySelectorAll)return{listMarkersSuppressed:false,suppressedCount:0};var styleTagId="gnr8-gallery-list-marker-reset-"+String(payload.moduleId);if(!document.getElementById(styleTagId)){var styleTag=document.createElement("style");styleTag.id=styleTagId;styleTag.textContent="#"+String(payload.moduleId)+" .gnr8-gallery-pages ul,#"+String(payload.moduleId)+" .gnr8-gallery-pages ol,#"+String(payload.moduleId)+" .gnr8-gallery-pages li{list-style:none;margin:0;padding:0;}#"+String(payload.moduleId)+" .gnr8-gallery-pages li::marker{content:none;display:none;}";(document.head||moduleEl).appendChild(styleTag);}var scopeNodes=[];if(pagesHost)scopeNodes.push(pagesHost);if(scopeNodes.length===0)scopeNodes.push(moduleEl);scopeNodes.forEach(function(scope){if(!scope||!scope.querySelectorAll)return;var lists=Array.prototype.slice.call(scope.querySelectorAll("ul,ol,li"));lists.forEach(function(node){if(!node||!node.style)return;node.style.listStyle="none";node.style.listStyleType="none";node.style.margin="0";node.style.padding="0";suppressedCount+=1;});});return{listMarkersSuppressed:suppressedCount>0,suppressedCount:suppressedCount};}
