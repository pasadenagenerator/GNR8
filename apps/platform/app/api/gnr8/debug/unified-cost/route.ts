...
import { getUnifiedCostOverview } from "@/gnr8/billing/unified-cost-view-service";
...
function mapError(error: unknown): { status: number; message: string } {
  ...
  if (
    ...
    message.includes("must be earlier than or equal")
  ) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const days = parseOptionalInt(search.get("days"));
    const limit = parseOptionalInt(search.get("limit"));
    const result = await getUnifiedCostOverview({
