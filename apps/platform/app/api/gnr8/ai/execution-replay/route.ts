...
import { runExecutionReplayEngineV1 } from "@/gnr8/ai/execution-replay-engine";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const pages: Array<{ slug: string } | Gnr8Page | { slug: string; page?: Gnr8Page }> = [];
    for (let i = 0; i < pagesRaw.length; i += 1) {
      ...
      pages.push({ slug, page: isGnr8Page(page) ? ({ ...page, slug } as Gnr8Page) : undefined });
    }

    const ledgerRaw = (body as any).ledger as unknown;
    if (!isRecord(ledgerRaw)) {
      return NextResponse.json({ error: "ledger must be an object" }, { status: 400 });
    }

    const waveIdRaw = typeof (body as any).waveId === "string" ? String((body as any).waveId).trim() : "";

    const result = await runExecutionReplayEngineV1({
      pages,
      ledger: ledgerRaw as AutonomousExecutionRuntimeLedgerV1,
