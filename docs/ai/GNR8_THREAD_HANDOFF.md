...
- Phase VCU-0 - Source Content & Visual Continuity Reality Audit is complete.
  ...
  VCU-0 was documentation and repository-reality analysis only. It audited why
  original ODV source body copy, headings, CTAs, logo candidate, source images,
  fonts, colors, visual style, source layout intent, and Workspace thumbnails
  do not yet meaningfully reach Generated Website Proposals. It changed no
  runtime, contract, persistence, schema, API, UI, worker, AI, WDB/WGP,
  Provider Payload, generation, proposal, thumbnail, publishing, deployment,
  DNS, or production behavior. VCU-1 has now completed the contract design.
...
- Phase MVP-2.0-L - Generation Contract Compliance v2 for ODV is complete
  and persisted latest compliance artifact
  `generation_contract_compliance_dfda0565997bd01266ec7464fcdeda0b`.
- Phase MVP-2.0-M - First Generation Evolution Analysis is complete for ODV
  and persisted latest evolution artifact
  `generation_evolution_analysis_89ab4005fcb11ef4d00682f7a86c1253`.
- Phase MVP-2.0-N - Generation Evolution Dashboard Architecture is complete
  and created the canonical read-only dashboard model in
...
- Phase WU-3 - Business Discovery Input Equivalence and Website Understanding
  Hardening is complete. The deterministic equivalence validator, read-only
...
- Target:
  `09dce7ea-d860-4f60-a1eb-26c3335b302e`.
- Relevant generated iterations:
  `generated_website_proposal_3f5cf8e9a4cd0cd91a3c7521edf8ddc3` and
  `generated_website_proposal_acbb2df2349e2973dbc7d26a696a378e`.
- Durable bundles:
  `generated_proposal_bundle_eb95bc58e327d009f2282cf6908dfdd4` and
  `generated_proposal_bundle_d43921f4457b6f26254bc8bf104c2075`.
- Original body copy is not lost at import. It is available upstream but first
  becomes unusable for generation at the WU-to-Business-Discovery/Business
  Discovery narrowing layer, then remains absent from DBT/BUR, WDB, WGP,
  Provider Payload, exports, and proposal bundles.
- ODV visual evidence already includes a strong source logo candidate
  `Tabla40x20cm_51.png`, hundreds of image/SVG assets, 17 font files including
  `Nationale` and `Fontello`, CSS color signals, screenshots, layout evidence,
  and section evidence. These remain evidence/candidates, not reviewed
  canonical identity.
- WDB/WGP currently describe governed intent and missing knowledge. They do
  not carry source-copy excerpts, source asset manifests, logo candidates,
  image candidates, font candidates, color candidates, visual-style candidates,
  source screenshot refs, or copy/asset transformation policy.
- Provider Payload v1/v2 serialize WGP/task envelopes and regeneration
  planning safely, but do not deliver source copy, asset files, preview URLs,
  fonts, colors, source screenshots, source HTML/CSS, or source continuity
  instructions.
- Iteration 1 and Iteration 2 proposals therefore preserve WGP constraints and
  missing-knowledge notices but use generic proposal copy, generic fonts/colors,
  and in Iteration 2 only abstract generated SVG evidence visuals.
- Workspace Original Website thumbnail gap: the card chooses the first imported
  asset preview instead of Evidence Capture screenshot evidence.
- Workspace Iteration 1/2 thumbnail gap: generated version cards set
  `previewImageHref` to `null` and rely on live iframes; no persisted
  screenshot thumbnail exists.
- Preferred near-term architecture: one combined pure Source Content & Visual
  Continuity projection over existing WU/import/evidence/candidate layers, not
  a new persisted canonical Creative/Brand artifact.
- Preferred thumbnail direction: hybrid persisted screenshot thumbnails for
...
- WU-2 remains complete and is the runtime foundation. It created the Source
  Website Understanding Projection as a deterministic, connector-neutral,
  evidence-backed, read-only projection of the current structured
  understanding of an imported source website.
- Preferred persistence policy: pure runtime projection with no dedicated
  persistence.
- Operator route:
  `/gnr8/admin/website-understanding/[siteVersionId]`.
- WU-2 real-target verification: ODV and ViroiDoc both produced valid,
...
- The dashboard is superadmin-only, server-rendered, and read-only.
- The dashboard consumes canonical artifacts from
  `siteVersion.importProvenanceSummary`; it is not a canonical artifact and
  does not create business truth.
- The preview boundary serves only allowlisted static proposal files rooted at
  `source/index.html`; previews are quarantined generated proposals, not
  published websites.
- No artifact editing, Business Alignment interaction, generation controls,
  regeneration controls, approval controls, publishing, deployment, DNS
  mutation, production mutation, provider execution, AI execution, workers,
...
- Canonical dashboard document:
  `docs/architecture/GENERATION_EVOLUTION_DASHBOARD.md`.
- The dashboard answers "What happened to my website over time?", not "What
  does my latest website look like?".
- Canonical timeline: Generation Cycle -> Iteration 1 -> Iteration 2 ->
  Iteration 3 -> ... -> Approved -> Published.
- Each iteration card documents iteration number, generation cycle, creation
  timestamp, status, overall assessment, recommendation, compliance status,
  confidence, and improvement summary.
- Each iteration card links to canonical artifacts such as Business Discovery,
  Digital Business Twin, Business Understanding Report, Business Alignment,
  Website Design Brief, Website Generation Package, Provider Payload,
  Generated Proposal, Observed Website, Compliance, Compliance Report,
  Improvement Plan, and Evolution Analysis.
- The preview model is Generated Website -> Preview URL -> Open Preview ->
  Static Snapshot -> Proposal Bundle.
- Every generated website remains permanently reachable through its iteration
  so users can manually inspect historical versions.
- Future metrics such as compliance trend, improvement velocity, regeneration
  gain, evidence growth, business confidence trend, iteration duration, and
  provider comparison are documentation-only concepts.
- No runtime behavior, persistence, UI implementation, routes, API, schema,
  workers, provider execution, AI execution, publishing, artifact contract
  changes, Generation Cycle runtime changes, compliance runtime changes, or
  automatic future-iteration comparison were added.

MVP-2.0-M prior evidence:
...
- Improved categories: `message_coverage`, `trust_signal_presence`,
  `constraints_preserved`.
- Regressed categories: none.
- Unresolved categories: `objectives_represented`,
  `navigation_obligations`, `page_obligations`, `section_obligations`,
  `asset_presence`, `accessibility_expectations_observable`,
  `seo_expectations_observable`.
- Metric deltas: compliant categories `+2`, partial categories `0`,
  non-compliant categories `-2`, findings `0`, deviations `-13`,
  evidence records `+13`, limitations `-16`, confidence `0`.
- Improvement Plan effectiveness:
  `Messages`, `Trust`, and `Constraints` observed improvement;
  `Accessibility`, `Assets`, `Business Positioning`, `Navigation`, `SEO`,
  and `Sections` had no demonstrated improvement.
- Latest reload, by-ID reload, and idempotent retry returned
  `generation_evolution_analysis_89ab4005fcb11ef4d00682f7a86c1253`.
- Source checks passed: both compliance artifacts exist, both target the same
  siteVersionId, both reference the same WGP, WGP lineage is preserved,
  Compliance v2 is latest, and neither source compliance artifact was
  modified.
- No compliance recomputation, Compliance Report v2, Improvement Plan v2,
  Provider Payload v3, regeneration, Business Approval, publishing,
  deployment, DNS mutation, production mutation, provider execution, AI
  execution, WGP mutation, OWM mutation, source compliance mutation,
...
- The Generation Evolution Dashboard is the canonical read-only historical
  view of a website's evolution across Generation Cycles and Iterations. It
  visualizes history, links to canonical artifacts and previews, and never
  edits artifacts, becomes business truth, publishes, or executes providers or
  AI.
