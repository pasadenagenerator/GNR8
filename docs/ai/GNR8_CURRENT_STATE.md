...
Current Phase: Phase 8B-12K-Retry-F6 — Worker-Accessible Source Delivery / Navigation Failure Diagnosis is complete.
Next recommended major phase: Phase 8B-12K-Retry-F7 — Worker-Accessible Raw Artifact Source Serving. Do not execute Limited Dry Run, Dry Run workers, simulation, reconstruction, AI generation, React generation, block generation, content generation, design token generation, UI write controls, approval workflow, publishing, source content mutation, domain/DNS mutation, CMS mutation, repair jobs, backfills, import retries, capture retries, migrations, or unrelated artifact generation without a separate explicit phase.
