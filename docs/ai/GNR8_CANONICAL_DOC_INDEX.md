...
## MVP-CUTLINE-13 Deployment Confirmation Intake

Canonical local-only human confirmation intake for the first single-site MVP rehearsal, recording manual production deployment/health status for `origin/main` `ba0d070cb77da5fb8fc3618469c567c5aeb4b356`, reported Vercel deployed commit `88c0a3b`, baseline database/Supabase/superadmin env presence, shadow flags missing/default-off, production Supabase target with unknown backup/restore posture, superadmin login/access confirmation, first rehearsal site `https://www.nogometne-tekme.si`, resulting commit-reconciliation, migration approval, online verification, dry-run, shadow-publish, and selected-ref blockers, while preserving the boundary of no deploys, Supabase migration applications, production/staging Supabase calls, provider/DNS/domain/billing/Stripe/Vercel/Openprovider actions, env flag changes, publish/shadow-publish, dry-run, online verification, route calls, or runtime mutation:
- `docs/product/gnr8-single-site-mvp-cutline-13-deployment-confirmation-intake.md`

## MVP-CUTLINE-14 Vercel Commit Reconciliation

Canonical local-only Git reconciliation of the manually reported Vercel deployed commit `88c0a3b` before migration execution, confirming it as `88c0a3b0dfa8a10ab3c94748b533e6664fc637cb` on `origin/codex/single-site-mvp-cutline-release`, not equal to or directly ancestor/descendant of `origin/main` `ba0d070cb77da5fb8fc3618469c567c5aeb4b356`, recording `migration_gate_blocked_wrong_deploy_ref`, preserving the independent `blocked_backup_restore_posture_unknown` migration blocker, and keeping online verification blocked, while preserving the boundary of no deploys, Supabase migration applications, production/staging Supabase calls, Vercel API/provider calls, env flag changes, dry-run, shadow-publish, online verification, runtime mutation, merge, rebase, hard reset, or push:
- `docs/product/gnr8-single-site-mvp-cutline-14-vercel-commit-reconciliation.md`

## MVP-CUTLINE-15 Production Deploy Ref Decision

Canonical local-only production deployment ref decision and migration approval posture record, confirming reported deployed SHA `88c0a3b0dfa8a10ab3c94748b533e6664fc637cb` as `on_release_branch_only`, recording no human approval to accept the release-branch production deploy and no human confirmation of a corrected `origin/main` deployment, setting exact outcome `production_ref_still_blocked`, migration gate `migration_gate_blocked_wrong_deploy_ref`, backup/restore posture `backup_restore_unknown`, and online verification blocked, while preserving the boundary of no deploys, redeploys, Vercel API/provider calls, Supabase migration applications, production/staging Supabase calls, env mutation, dry-run, shadow-publish, online verification, runtime/app/service/SQL behavior changes, commit, push, merge, rebase, hard reset, or branch deletion:
- `docs/product/gnr8-single-site-mvp-cutline-15-production-deploy-ref-decision.md`

## MVP-CUTLINE-16B Supabase Pro Upgrade Pending

Canonical documentation-only record of the human decision to upgrade production Supabase to Pro before migrations because Supabase Free Plan currently reports project backups unavailable, setting backup/restore posture `backup_restore_pending_pro_upgrade`, migration gate `migration_gate_blocked_waiting_for_pro_backup_confirmation`, preserving migration approval as `not_approved`, documenting the exact required human follow-up to upgrade Pro, open Supabase Dashboard > production project > Database > Backups, confirm at least one visible backup or visible backup/PITR capability, and report backup status before migrations, while keeping online verification blocked and preserving the boundary of no deploys, migrations, Supabase calls, Vercel/provider calls, env mutation, dry-run, shadow-publish, online verification, runtime/app/service/SQL behavior changes, commit, or push:
- `docs/product/gnr8-single-site-mvp-cutline-16b-supabase-pro-upgrade-pending.md`

## MVP-CUTLINE-16C Pro Backup Confirmation Intake

Canonical documentation-only intake for human-provided production Supabase Pro backup confirmation, recording production organization/project on Pro plan, production project Database > Backups page, visible scheduled backups, multiple physical backups with Restore buttons, latest visible backup `17 Aug 2026 03:08:21 (+0000)`, backup/restore posture `backup_restore_confirmed`, storage caveat that Supabase Storage objects are not included in database backups, migration gate `migration_gate_blocked_wrong_deploy_ref_backup_confirmed`, migration approval `not_approved`, and online verification blocked, while preserving the boundary of no deploys, migrations, Supabase calls, Vercel/provider calls, env mutation, dry-run, shadow-publish, online verification, runtime/app/service/SQL behavior changes, commit, or push:
- `docs/product/gnr8-single-site-mvp-cutline-16c-pro-backup-confirmation-intake.md`

## MVP-CUTLINE-17 Deploy Ref Resolution And Migration Plan

Canonical documentation-only production deployment ref resolution record and migration execution plan, confirming local HEAD and `origin/codex/single-site-mvp-cutline-release` at `88c0a3b0dfa8a10ab3c94748b533e6664fc637cb`, `origin/main` at `ba0d070cb77da5fb8fc3618469c567c5aeb4b356`, no new Vercel deployment SHA, no explicit human confirmation of production redeployed to main, no explicit acceptance of the release-branch production ref, resulting production decision `production_ref_still_blocked`, migration gate `migration_gate_blocked_wrong_deploy_ref_backup_confirmed`, migration approval `not_approved`, backup/restore posture `backup_restore_confirmed`, online verification blocked, and an exact future-only 18-migration execution plan with preflight checks, stop criteria, post-migration verification, and the Supabase Storage restore caveat, while preserving the boundary of no deploys, migrations, Supabase calls, Vercel/provider calls, env mutation, dry-run, shadow-publish, online verification, runtime/app/service/SQL behavior changes, commit, or push:
- `docs/product/gnr8-single-site-mvp-cutline-17-deploy-ref-resolution-and-migration-plan.md`

## MVP-CUTLINE-18B Production Deploy Ref Confirmed

Canonical documentation-only production deploy-ref confirmation record for the single-site MVP cutline, recording human confirmation that both `gnr8-platform` and `gnr8-worker` production projects deploy from `main` at `ba0d070`, local Git confirmation that `ba0d070` resolves to `origin/main` `ba0d070cb77da5fb8fc3618469c567c5aeb4b356`, current branch/head and release-branch refs at `88c0a3b0dfa8a10ab3c94748b533e6664fc637cb`, production ref decision `production_ref_corrected_to_main`, backup/restore posture `backup_restore_confirmed`, latest visible backup `17 Aug 2026 03:08:21 (+0000)`, Supabase Storage backup caveat, migration gate `migration_gate_ready_for_execution_approval`, migration approval `not_approved`, and online verification `blocked_until_migrations_applied_and_verified`, while preserving the boundary of no deploys, migrations, Supabase calls, Vercel/provider calls, env mutation, dry-run, shadow-publish, online verification, runtime/app/service/SQL behavior changes, commit, or push:
- `docs/product/gnr8-single-site-mvp-cutline-18b-production-deploy-ref-confirmed.md`

## MVP-CUTLINE-19 Migration Execution Approval Intake

Canonical documentation-only migration execution approval intake for the single-site MVP cutline, recording that the exact human approval sentence was present, setting migration approval to `migration_execution_approved_pending_run`, setting the next migration gate to `migration_execution_ready`, confirming all 18 required migration files exist locally from `20260722120000_aaf_persistence_core.sql` through `20260806120000_single_site_publish_operator_action_audit.sql`, preserving backup/restore posture `backup_restore_confirmed`, latest visible backup `17 Aug 2026 03:08:21 (+0000)`, and Supabase Storage backup caveat, and keeping online verification `blocked_until_migrations_applied_and_verified`, while preserving the boundary of no deploys, migrations, Supabase calls, Vercel/provider calls, env mutation, dry-run, shadow-publish, online verification, runtime/app/service/SQL behavior changes, commit, or push:
- `docs/product/gnr8-single-site-mvp-cutline-19-migration-execution-approval-intake.md`
