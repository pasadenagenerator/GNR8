...
- [x] Confirm MVP-CUTLINE-62 refreshed governed dry-run readback is recorded with `ok=true`, resolver `complete`, wrapper `dry_run_ready`, gate `allowed`, selected active pointer absent, and CUTLINE-62 shadow-publish action count `0` before any shadow-publish retry.
- [x] Confirm MVP-CUTLINE-63 controlled shadow-publish retry is recorded with exactly one CUTLINE-63 shadow-publish action, resolver `complete`, wrapper `published_via_existing_orchestrator`, active pointer `0 -> 1`, and no provider/DNS/domain/billing/Stripe/Openprovider mutation before public verification and MVP closeout.
- [x] Confirm MVP-CUTLINE-64 post-shadow-publish public verification is recorded with active pointer still targeting `a3f9493e-9da4-4ef8-8608-154fe6d25a0f`, candidate `PUBLISHED`, artifact `production`, public `https://www.chs.si/` HTTP `200`, platform/worker health green, visually nonblank public screenshot, and final one-site internal MVP rehearsal acceptance pending 20-site validation.
- [x] Confirm Airship MVP Recovery CHS demo visual acceptance is recorded with manual verdict `visual demo accepted for internal review`, GNR8 demo URL `https://chs-airship.app.pasadenagenerator.com/`, active pointer target `92e476b9-67fc-408a-be3d-5c744aa0f3f6 / 5ac3716a-f29d-4648-bc86-a6942638ed53`, external `https://www.chs.si/` explicitly `external / not cut over`, and no promote-to-live, rollback, dry-run, shadow-publish, source-capture, DNS/domain/provider/billing/env, artifact/page-content, or active pointer mutation by the readback.
- [x] Confirm Airship MVP Recovery ARIS demo visual acceptance is recorded with status `mvp_recovery_aris_demo_visual_accepted`, GNR8 preview URL `https://aris-airship.app.pasadenagenerator.com/`, migration `ebf62324-1e51-4435-abd7-004722fb48d6`, runtime site `site_6b859cc1599a5b6642dc`, draft `5ed1b4da-eb06-4eee-bdc0-5b5cdec99707 / 1`, candidate `6d712ab9-f48e-49a3-9c26-03915365d746`, artifact `8073651e-510b-47e7-8363-8a742b7967db`, preview binding `e73e5d73-cbbe-4ff7-9cdb-75c0ee6aa2ae`, candidate state `DRAFT`, artifact stage `shadow`, active pointer `null / no row`, external `https://www.aris.si/` explicitly external/untouched, and no DNS cutover, live customer publish, promote, rollback, dry-run, shadow-publish, source-capture, provider/DNS/domain/billing/env, artifact/page-content, runtime, or active pointer mutation by the readback.
...
CUTLINE-60 remediation diagnosis result: exact approval sentence was present, but the task stopped before mutation because no existing safe workflow can remediate this approved candidate's missing page-level governance from source truth. Read-only production readback confirmed candidate `a3f9493e-9da4-4ef8-8608-154fe6d25a0f` remains `APPROVED`, bound to artifact `1f80138a-39c2-4210-ac61-16200e5a2254`, with page governance `0/1`; sibling runtime versions are also `0/1`, and raw imported/template artifact evidence for the candidate resolved to `null`. The existing runtime-store helper `materializePageMigrationGovernanceForSiteVersion(...)` is available, but the only existing canonical reconstruction caller requires verified raw/imported artifact evidence, while the improved-candidate adapter currently clones page governance from the clone version and would preserve the gap. No governed dry-run rerun, shadow-publish, runtime publish, rollback, active pointer switch, provider/DNS/domain/billing/Stripe/Openprovider mutation, migration, env mutation, deploy, commit, or push occurred. Selected active pointer stayed `0 -> 0`. Closeout: `docs/product/gnr8-single-site-mvp-cutline-60-page-migration-governance-remediation.md`.

CUTLINE-61 source-owned page governance remediation result: exact approval sentence was present, local workflow/tests were added, focused tests and touched-file TypeScript passed, and production page-level `migration_governance` was materialized for the approved `chs.si` candidate through `materializePageMigrationGovernanceForSiteVersion(...)`. Candidate `a3f9493e-9da4-4ef8-8608-154fe6d25a0f` remains `APPROVED`, artifact binding remains `1f80138a-39c2-4210-ac61-16200e5a2254`, page governance moved `0/1 -> 1/1`, required governance fields and source-owned identity match, and selected active pointer stayed `0 -> 0`. Remediation audit row `8a4d4356-4078-42c2-96e6-aa57824444ef` affected one page. Governed dry-run rerun action `3fdef831-4897-4605-9f17-6ae00c888894` completed `ok=false`, wrapper `preflight_blocked`, resolver `incomplete`, with blockers `publish_activation_gate_input_watermark_mismatch`, `publish_activation_gate_stale`, `publish_activation_handoff_watermark_mismatch`, `publish_target_ref_mismatch`, and `single_site_publish_wrapper_resolver_incomplete`. No shadow-publish, runtime publish, rollback, active pointer switch, provider/DNS/domain/billing/Stripe/Openprovider mutation, migration, env mutation, deploy, commit, push, new AAF request/decision, or new gate attempt occurred. Shadow-publish eligibility is not restored; refresh the publish metadata chain after committing/deploying the remediation workflow. Closeout: `docs/product/gnr8-single-site-mvp-cutline-61-source-owned-page-governance-remediation.md`.

CUTLINE-62 cleanup and final readback result: after the successful refreshed governed dry-run at supplied commit/deploy SHA `4ec3c575d1e92ce4b9c4488b534ed2b5f4c15541`, cleanup/readback only confirmed no leftover `tmp-cutline62` files, exactly one CUTLINE-62 dry-run action `ad9660dd-c474-431c-a4ef-f935302c946b`, `ok=true`, wrapper `dry_run_ready`, resolver `complete`, gate `cee202bb-e99c-4b5b-8839-876a54a1ba35` `allowed`, selected runtime active pointer `0`, candidate/artifact pointer counts `0/0`, and CUTLINE-62 shadow-publish action count `0`. No metadata refresh rerun, dry-run rerun, shadow-publish, runtime publish, rollback, active pointer mutation, provider/DNS/domain/billing/Stripe/Openprovider mutation, migration, env mutation, deploy, production data mutation, or production cleanup write occurred. Online verification status is `ready_for_shadow_publish_retry_pending_fresh_approval`. Closeout: `docs/product/gnr8-single-site-mvp-cutline-62-cleanup-and-final-readback-closeout.md`.

CUTLINE-63 controlled shadow-publish retry result: exact approval was present and one production shadow-publish retry was run through the existing governed shadow-publish operator route handler. Production preflight confirmed candidate `a3f9493e-9da4-4ef8-8608-154fe6d25a0f` `APPROVED`, artifact `1f80138a-39c2-4210-ac61-16200e5a2254` bound, page `migration_governance` `1/1`, CUTLINE-62 dry-run `ad9660dd-c474-431c-a4ef-f935302c946b` `ok=true` / resolver `complete` / wrapper `dry_run_ready`, gate `cee202bb-e99c-4b5b-8839-876a54a1ba35` `allowed`, selected active pointer absent, and CUTLINE-63 shadow-publish action count `0`. The retry created action `6c44f0ac-5546-448b-9e04-a07aa179f92f`, returned `ok=true`, route `shadow_publish_completed`, resolver `complete`, wrapper `published_via_existing_orchestrator`, orchestrator `called`, and active pointer `0 -> 1` targeting `site_57d9665a3a5867edf6ef -> a3f9493e-9da4-4ef8-8608-154fe6d25a0f`. Runtime candidate/artifact state moved through the existing orchestrator to `PUBLISHED` / `production`. AAF request/decision/gate counts, provider operation approvals, provider handoffs/jobs, runtime host bindings, runtime domain host bindings, publish targets, rollback table state, and billing table state stayed unchanged; no rollback, second shadow-publish, dry-run rerun, provider/DNS/domain/billing/Stripe/Openprovider mutation, migration, env mutation, deploy, or unrelated code change occurred. Public/read-only URL checks returned HTTP `200` for `https://www.chs.si/`, `https://app.pasadenagenerator.com/`, and worker health. Online verification status is `shadow_publish_completed_pending_public_verification_and_mvp_closeout`. Closeout: `docs/product/gnr8-single-site-mvp-cutline-63-controlled-shadow-publish-retry.md`.

CUTLINE-64 post-shadow-publish public verification result: read-only production verification on 2026-08-31 confirmed active pointer `site_57d9665a3a5867edf6ef -> a3f9493e-9da4-4ef8-8608-154fe6d25a0f`, active artifact `1f80138a-39c2-4210-ac61-16200e5a2254`, candidate state `PUBLISHED`, and artifact publish stage `production`. CUTLINE-63 action `6c44f0ac-5546-448b-9e04-a07aa179f92f` remains the latest operator action, CUTLINE-63 shadow-publish action count is `1`, operator actions after CUTLINE-63 are `0`, and shadow-publish actions after CUTLINE-63 are `0`. AAF requests/decisions/gates remain `8/8/3`; provider approvals/handoffs/jobs remain `2/2/0`; runtime host bindings/domain host bindings remain `22/4`; rollback and billing subscription tables remain absent. Public `https://www.chs.si/` returned HTTP `200` with title `Home | CHS` and substantial CHS body content; `https://app.pasadenagenerator.com/` returned HTTP `200`; worker health returned HTTP `200`, `ok=true`, `status=ready`. Browser screenshot `/private/tmp/gnr8-cutline64-chs-public-screenshot.png` was visually nonblank. No rollback, second shadow-publish, active pointer mutation, runtime publish mutation, provider/DNS/domain/billing/Stripe/Openprovider mutation, migration, env mutation, app/runtime code change, broad refactor, broad test sweep, or manual deploy occurred. Online verification status is `one_site_internal_mvp_rehearsal_accepted_pending_20_site_validation`. Closeout: `docs/product/gnr8-single-site-mvp-cutline-64-post-shadow-publish-public-verification-closeout.md`.

Airship MVP Recovery CHS demo visual acceptance result: Gregor manually reviewed GNR8-controlled demo URL `https://chs-airship.app.pasadenagenerator.com/` on 2026-09-15 and accepted it as visually OK for internal demo, with verdict `visual demo accepted for internal review`. The manual review observed no fallback/debug render, no `FALLBACK PREVIEW`, no `raw-block`, no `CAPTURE_DRIVEN`, no diagnostics text, and a coherent branded landing page with clear hero, sections, CTA/contact area, and acceptable GNR8/Airship improved direction. The active pointer target remains `92e476b9-67fc-408a-be3d-5c744aa0f3f6 / 5ac3716a-f29d-4648-bc86-a6942638ed53`, host binding remains `89b2cafa-651a-4402-a947-0c3d45378a3d / ACTIVE / shadow / site_57d9665a3a5867edf6ef`, and external `https://www.chs.si/` remains `external / not cut over`. No promote-to-live, rollback, dry-run, shadow-publish, source-capture, provider/DNS/domain/billing/env mutation, artifact/page-content mutation, or active pointer mutation occurred in this readback. Status is `mvp_recovery_chs_airship_demo_visual_accepted`. Closeout: `docs/product/gnr8-airship-mvp-recovery-chs-demo-readiness-closeout.md`.

Airship MVP Recovery ARIS demo visual acceptance result: Gregor manually reviewed GNR8-controlled preview URL `https://aris-airship.app.pasadenagenerator.com/` on 2026-09-16 and accepted it as visually OK for internal demo, making ARIS the second imported-site demo target. The preview uses binding `e73e5d73-cbbe-4ff7-9cdb-75c0ee6aa2ae` on hostname `aris-airship.app.pasadenagenerator.com` for runtime site `site_6b859cc1599a5b6642dc`; candidate `6d712ab9-f48e-49a3-9c26-03915365d746` remains `DRAFT`, artifact `8073651e-510b-47e7-8363-8a742b7967db` remains `shadow`, and active pointer remains `null / no row`. External `https://www.aris.si/` remains external and untouched. This is internal demo acceptance only, not DNS cutover or live customer publish. No promote-to-live, rollback, dry-run, shadow-publish, source-capture, provider/DNS/domain/billing/env mutation, artifact/page-content mutation, runtime mutation, AI provider call, or active pointer mutation occurred in this readback. Status is `mvp_recovery_aris_demo_visual_accepted`. Closeout: `docs/product/gnr8-airship-mvp-recovery-aris-demo-visual-acceptance-closeout.md`.
