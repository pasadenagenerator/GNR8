...
- [ ] Confirm MVP-CUTLINE-62 refreshed governed dry-run readback is recorded with `ok=true`, resolver `complete`, wrapper `dry_run_ready`, gate `allowed`, selected active pointer absent, and CUTLINE-62 shadow-publish action count `0` before any shadow-publish retry.
