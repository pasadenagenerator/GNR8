...
function parseIsoOrNull(value, field) {
...
export class SuperadminTrialService {
    ...
    async updateOrgTrial(input) {
        ...
        if (body && typeof body === 'object' && 'trialEndsAt' in body) {
            const trialEndsAt = parseIsoOrNull(body.trialEndsAt, 'trialEndsAt');
            const trialStartedAt = 'trialStartedAt' in body
                ? parseIsoOrNull(body.trialStartedAt, 'trialStartedAt')
                : null;
        ...
        const days = parsePositiveDays(body?.days, 14);
